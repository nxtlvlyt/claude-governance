...
OPEN ITEM (2026-07-28, operator-raised, blocked on nxtbeast reachability): LAGUNA UPDATE AVAILABLE.
Upstream verified via ollama.com search: laguna-xs-2.1 updated 3 days ago, laguna-s-2.1 yesterday.
nxtbeast unreachable at check time (ssh + ping fail), so installed digests could not be compared.
WHEN NXTBEAST IS BACK: (1) record current laguna-xs-2.1:q8_0 digest as rollback pin (/api/show),
