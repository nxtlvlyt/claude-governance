...
import { healCloud } from './seat_dispatch.mjs';
...
const p1 = healCloud(E('TIMEOUT', 'timeout 180000ms'), body, 0, {});                 // 0 prior timeouts
ck(p1 && p1.extendTimeout === true, 'TIMEOUT heal #1: extend+retry (one healing chance preserved)');
const p2 = healCloud(E('TIMEOUT', 'timeout 360000ms'), body, 1, { TIMEOUT: 1 });     // 1 prior timeout
...
// mirrors dispatchWithWaterfall: plan = healCloud(...); kindCounts[kind]++ ; null => break.
function simulate(errKindSeq, MAX = 3) {
  ...
  for (let heal = 0; heal <= MAX; heal++) {
    ...
    const plan = healCloud(e, body, heal, kindCounts);
...
const p400 = healCloud(E('HTTP_400', '400: context length exceeded'), big, 0, {});
...
const p400b = healCloud(E('HTTP_400', '400: context length exceeded'), p400.body, 1, { HTTP_400: 1 });
...
const pThink = healCloud(E('EMPTY_CONTENT_THINKING', 'reasoning consumed budget'), body, 0, {});
...
const pMsg = healCloud(E('API_ERROR', 'num_ctx too long, exceed maximum'), big, 0, {});
