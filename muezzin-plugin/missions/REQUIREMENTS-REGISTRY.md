...
|---|---|---|---|---|
| A1 | POI/roadside info richness (oddities, roadsides had no useful info) | poi-tags, poi-services-nearby, poi-hover-preview-cards | **NOT-DONE (operator-confirmed live 2026-07-02)** | AUDIT CORRECTION: the 1st fix was WRONG AT THE DATA LAYER — it rendered `m.description`/`m.why_cool`/`m.detour_note`, fields that DO NOT EXIST in production meta (real schema, verified via live pois.json: quirky_rating, photo_stop, walk_out, detour_minutes, source_url) — so every oddity fell to the generic fallback; operator saw it live. Its "live-verified" claim only checked code presence, never one real POI's data → verification theater. 2nd fix fcc53e2 renders the REAL fields (verified by simulating with actual prod data) — committed + mirrored, **NOT YET DEPLOYED** (classifier requires operator deploy auth). charging/locker branches under same audit now. Lesson: verify a popup fix by rendering ONE REAL POI's data, not by checking the branch exists. |
