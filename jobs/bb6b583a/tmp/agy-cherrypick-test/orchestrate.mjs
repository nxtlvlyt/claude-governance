...
import { implementStep, isProseTarget, executorFloorSeat } from './executor.mjs';
...
function readCheckpoint(cwd, missionId, missionHash) {
  try {
    ...
    // AMENDMENT INVALIDATION (parent QUEUE item 20, paid 2026-07-11 twice in one day — the FORK's
    // own receipt is atv-11-design-pass.S1.S1: step 2 resumed a stale pre-amendment css bank, the
    // amended .card-grid validation never re-ran, and the site shipped unstyled past a DOM-count
    // witness (repaired by atv-11b-vocab-bridge)): a checkpoint is only trustworthy for the EXACT
    // mission text it was written under. The trust-boundary sha check cannot catch this (the old
    // commit genuinely touches the target — it is the step SPEC that changed). Hash mismatch OR a
    // hashless legacy checkpoint (pre-fix era, provenance unprovable) -> invalidate; steps re-run.
...
export function artifactFilesFor(steps, cwd) {
  // P2e fix (2026-07-10, receipted 2x — atv-7b + atv-7.S2 panel REVISEs: "artifact bundle
  // omits <the mission's own file> and instead contains unrelated repo docs (QUEUE.md,
  // STATE.md)"): engine-exec command/verify steps record `target` (SINGULAR) — only [edit]
  // steps carry `targets` (plural). Collecting only the plural left command-class missions
  // with an empty list, so the *.md-fallback swept random repo-root docs into the panel
  // bundle in code-repo missions (where cwd is the repo root). Collect both shapes.
...
// REPAIR-DEFAULT OPTIONS (atv-1.S1 repair-path fix, 2026-07-08 — receipted live: the repair/heal
// path fires on EVERY validation-failed emission, i.e. every retry). PURE decision of what options
// orchestrate's OWN default repairFn (built only when the caller did not inject one — see
// repairFnInjected below) hands to makeRepairFn (repair.mjs). Extracted so the mode-honoring
// behavior is directly unit-testable without a live dispatch.
//   codeRepo=true  -> UNCHANGED prior behavior, byte-for-byte: {codeRepo, repoRoot, allowFiles},
//     no explicit model. Code-repo repair targets are source files inside an existing repo, not
//     prose, and executorFloorSeat's prose/frontier routing was never audited for that path — this
//     fix is scoped to the non-code (research/prose) path the bug was receipted on; repair.mjs's
//     own 'qwen3-coder-next' fallback (its LAST-resort default, untouched by this fix) still decides.
//   codeRepo=false -> { model: executorFloorSeat(target) }: the SAME mode-aware floor primary
//     emission already uses (executor.mjs) — so a mission running under an active mode (e.g.
//     agy-hybrid) gets that mode's executor seat on repair too, instead of always landing on
//     repair.mjs's hardcoded default regardless of the active seat_modes.mjs mode.
export function repairDefaultOpts(codeRepo, repoRoot, allowFiles, target) {
  return codeRepo ? { codeRepo, repoRoot, allowFiles } : { model: executorFloorSeat(target) };
}
...
export async function orchestrate(mission, cwd, {
  ...
  // repairFn INJECTION FLAG (atv-1.S1 repair-path fix, 2026-07-08): the default used to be
  // built HERE, ONCE per mission, before any step's target_files were known — so it could
  // never be target-aware, and for a non-code-repo mission it passed makeRepairFn NO model
  // at all, silently falling through to repair.mjs's hardcoded 'qwen3-coder-next' default
  // regardless of the active seat_modes.mjs mode (receipted live: agy-hybrid's glm-5.2 —
  // 38/0/0 track record — bypassed on every repair/heal retry). The real default is now
  // built per-step inside the escalation loop below (tierRepairFn), where `target` is in
  // scope, using executorFloorSeat — the SAME mode-aware floor primary emission already
  // uses (executor.mjs). This flag just remembers whether the CALLER supplied its own
  // repairFn (every offline test does) so that per-step build never overrides a test mock.
  const repairFnInjected = !!repairFn;
  ...
  for (const step of queue.steps) {
    ...
    // NON-ESCALATED DEFAULT (atv-1.S1 repair-path fix, 2026-07-08): when the caller did not inject
    // its own repairFn (see repairFnInjected above), build the default HERE — per step, per
    // escalation tier — so repairDefaultOpts can key off THIS step's real `target`.
    const tierRepairFn = escModel
      ? makeRepairFn(writeRoot, { model: escModel, ...(codeRepo ? { codeRepo, repoRoot, allowFiles } : {}) })
      : (repairFnInjected ? repairFn : makeRepairFn(writeRoot, repairDefaultOpts(codeRepo, repoRoot, allowFiles, target)));
    ...
    if (escalationOn && stepResult && !structuralSplit && ESCALATION_SUBSTANTIAL.has(lastFailReason)) {
      ...
      // atv-1.S1 fix (2026-07-08): was a second hardcoded 'sonnet' for prose — now that the mode
      // seat outranks the floor for frontier executors (executorFloorSeat), the hardcode would
      // wrongly skip the sonnet ladder rung (straight to opus) on modes like agy-hybrid where
      // tier 0 actually ran glm-5.2. Ask the ONE shared floor decision instead. CODE targets keep
      // null (badal-chosen local seat) byte-for-byte.
      const usedSeat = escModel || (isProseTarget(target) ? executorFloorSeat(target) : null);
...
if (process.argv[1]?.endsWith('orchestrate.mjs')) {
  ...
  // sandbox's .md files are the artifacts, newest first; underscore-prefixed excluded.
  {
    ...
    ck(JSON.stringify(artifactFilesFor([{ ok: true, target: 'y.md', engineExec: true }], adir)) === '["y.md"]',
      'artifactFilesFor: SINGULAR engine-exec `target` collected too (P2e — command/verify steps no longer fall through to the repo-doc-sweeping fallback)');
    ck(JSON.stringify(artifactFilesFor([{ ok: true, targets: ['x.md'] }, { ok: true, target: 'y.md' }], adir)) === '["x.md","y.md"]',
      'artifactFilesFor: mixed plural+singular steps merge, deduped');
  ...
  // a failing witness leaves HEAD unchanged and the repo clean.
  {
    ...
    // must refuse the commit itself, never rely on an [edit]-type path that this step never uses.
    {
      ...
      // GOTCHA (found live): a pre-modification made BEFORE orchestrate() runs is wiped by
      // resetAllowFiles(), which restores dirty ALLOW-FILES to HEAD as the mission's own clean-
      // baseline guarantee. The gutting must happen INSIDE a mission step, same as the real
      // 44da372 shape (an earlier step corrupts the file; a later step blindly commits it).
      const mission = `MISSION-CLASS: code-repo\nREPO-ROOT: ${repo}\nALLOW-FILES:\n  - docs/BIG.md\nMaqsad: x. Done means: y.`;
      const q = { mission_id: 'CR4', steps: [
        { step_index: 1, description: 'gut the file (simulates a botched prior step)', action_type: 'command', target_files: ['docs/BIG.md'], context_dependencies: [], validation_command: "Set-Content -Path docs/BIG.md -Value gutted" },
        { step_index: 2, description: 'commit the change', action_type: 'command', target_files: ['docs/BIG.md'], context_dependencies: [], validation_command: 'git add docs/BIG.md; git commit -q -m gut --no-verify' },
      ] };
      ...
      ck(res.ok === false && res.steps.at(-1)?.reason === 'undeclared-shrinkage', 'code-repo e2e (4) COMMAND-STEP SHRINKAGE: a [command] step committing an already-gutted file is REFUSED (undeclared-shrinkage), not silently committed');
    ...
    // (5) COMMAND-STEP PARITY, NEGATIVE CONTROL — a legitimate small/declared change still commits.
    {
      ...
      const before = head(repo);
      const mission = `MISSION-CLASS: code-repo\nREPO-ROOT: ${repo}\nALLOW-FILES:\n  - docs/BIG2.md\nMaqsad: x. Done means: y.`;
      const q = { mission_id: 'CR5', steps: [
        { step_index: 1, description: 'grow the file (legitimate change)', action_type: 'command', target_files: ['docs/BIG2.md'], context_dependencies: [], validation_command: "Add-Content -Path docs/BIG2.md -Value 'one more line'" },
        { step_index: 2, description: 'commit the change', action_type: 'command', target_files: ['docs/BIG2.md'], context_dependencies: [], validation_command: 'git add docs/BIG2.md; git commit -q -m grow --no-verify' },
      ] };
      const res = await orchestrate(mission, sbx, { deconstructFn: async () => ({ ok: true, queue: q }), implementFn: async () => { throw new Error('EXECUTOR DISPATCHED FOR COMMAND STEP'); }, maxRepairs: 0, verdictFn: approveVerdict, witnessFn: okWitness });
  ...
  // daemon re-run the WHOLE mission from step 1, discarding completed steps (the KB churn).
  {
    ...
    // and SKIPS the committed steps — steps 1-2 are NOT re-run, the mission resumes from step 3.
    {
      ...
      // AMENDED MISSION INVALIDATION (parent QUEUE item 20, 2026-07-11 — this FORK's receipt is
      // atv-11-design-pass.S1.S1: step 2 resumed a stale pre-amendment css bank and the site
      // shipped unstyled): a checkpoint written under one mission text must NOT resume a run of
      // a DIFFERENT (amended) text, even when every sha is valid — the step SPEC changed.
      const run4Calls = {};
  ...
  // ---- REPAIR/HEAL PATH HONORS THE ACTIVE MODE FLOOR (atv-1.S1 repair-path fix, 2026-07-08 —
  // receipted live: the repair/heal path fires on EVERY validation-failed emission, i.e. every
  // retry, and used to ALWAYS dispatch repair.mjs's hardcoded 'qwen3-coder-next' default
  // regardless of the active seat_modes.mjs mode — even when the mode's executor was explicitly
  // e.g. agy-hybrid's glm-5.2 (38/0/0 track record), silently bypassed on every repair retry).
  // (a) repairDefaultOpts (the pure decision orchestrate.mjs's own default repairFn build now
  // uses) picks the MODE's executor floor for a non-code-repo mission, NOT the hardcoded default
  // — proven directly, no live dispatch. (b) codeRepo=true PRESERVES the prior behavior
  // byte-for-byte (no explicit model — repair.mjs's own fallback still decides; out of scope for
  // this fix per the receipt). (c) feeding repairDefaultOpts's output into the REAL makeRepairFn
  // (repair.mjs, mocked dispatch only) proves the seat actually DISPATCHED for a repair carries
  // the mode's model — the exact mechanism the bug broke.
  {
    const { pickSeat: ps2 } = await import('./seat_modes.mjs');
    const withMode2 = async (env, fn) => {
      const saved = process.env.MUEZZIN_MODE;
      if (env) process.env.MUEZZIN_MODE = env; else delete process.env.MUEZZIN_MODE;
      try { return await fn(); } finally { if (saved === undefined) delete process.env.MUEZZIN_MODE; else process.env.MUEZZIN_MODE = saved; }
    };
    // (a) agy-hybrid's executor is glm-5.2 (a censused cloud/frontier-tag seat) -> executorFloorSeat
    // returns it regardless of target extension; repairDefaultOpts must carry it as `model`.
    const agyOpts = await withMode2('agy-hybrid', async () => repairDefaultOpts(false, null, [], 'repair-floor-target.mjs'));
    ck(agyOpts.model === 'glm-5.2', `REPAIR-FLOOR (a): non-code-repo repair default under agy-hybrid mode carries model=glm-5.2 (the MODE's executor seat), not the hardcoded qwen3-coder-next — got ${JSON.stringify(agyOpts)}`);
    // absent mode -> today's mode-floor default (north-mini-code-toolcall) — STILL not a silent
    // empty-options fallthrough; the fix is "always pass an explicit model", not "match old value".
    const noModeOpts = await withMode2('__none__', async () => repairDefaultOpts(false, null, [], 'x.mjs'));
    const expectFloor = await withMode2('__none__', async () => ps2('executor', 'north-mini-code-toolcall'));
    ck(noModeOpts.model === expectFloor, 'REPAIR-FLOOR (a2): no active mode -> repair default still carries the executor FLOOR model explicitly (never an empty options object)');
    // (b) codeRepo=true is BYTE-FOR-BYTE preserved: no model key, own options only.
    const crOpts = repairDefaultOpts(true, '/some/repo', ['a.mjs'], 'a.mjs');
    ck(!('model' in crOpts) && crOpts.codeRepo === true && crOpts.repoRoot === '/some/repo' && JSON.stringify(crOpts.allowFiles) === JSON.stringify(['a.mjs']),
      'REPAIR-FLOOR (b): codeRepo=true repair default UNCHANGED (codeRepo/repoRoot/allowFiles only, no model override) — code-repo repair behavior is PRESERVED, not touched by this fix');
    // (c) END-TO-END THROUGH THE REAL makeRepairFn (repair.mjs) with a mocked dispatch only:
    // proves the seat DISPATCHED for a repair carries the mode's model, not qwen3-coder-next.
    let sawModel = null;
    const capDispatch = async (seat) => { sawModel = seat.model; return { content: '```\nexport const healed = 1;\nARTIFACT-COMPLETE\n```' }; };
    const realRepairFn = await withMode2('agy-hybrid', async () => makeRepairFn(dir, { ...repairDefaultOpts(false, null, [], 'repair-floor-target.mjs'), dispatch: capDispatch }));
    fs.writeFileSync(path.join(dir, 'repair-floor-target.mjs'), 'broken\n');
    await realRepairFn({ step_index: 1, description: 'repair', target_files: ['repair-floor-target.mjs'], validation_command: 'node -c repair-floor-target.mjs' }, { ok: false, out: 'SyntaxError' });
    ck(sawModel === 'glm-5.2', `REPAIR-FLOOR (c): the REAL makeRepairFn dispatched seat.model=glm-5.2 under agy-hybrid mode (was always qwen3-coder-next before this fix) — got ${sawModel}`);
    ck(fs.readFileSync(path.join(dir, 'repair-floor-target.mjs'), 'utf8').includes('healed = 1'), 'REPAIR-FLOOR (c): the repair actually wrote the healed content (deed, not just a captured model name)');
  }
