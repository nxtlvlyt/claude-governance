#!/usr/bin/env python3
"""
operator-context.md review chain runner.
Runs the full 6-agent deliberation stack against operator-context.md,
verifying whether the document accurately describes the current substrate.

Usage:
  python scripts/chain-review.py 1    -- phase 1 (gemma + qwen)
  python scripts/chain-review.py 2    -- phase 2 (laguna + granite + nemotron)
  default phase: 1

Between phase 1 and phase 2, Seat 3 (claude-sonnet-4-6) writes synthesis to:
  <TEMP>/chain-review/sonnet-synthesis.txt
...
OPCTX_PATH   = os.path.join(CLAUDE_HOME, "operator-context.md")
RIJAL_PATH   = os.path.join(CLAUDE_HOME, "canon", "model-rijal.md")
OUTPUT_DIR   = os.path.join(tempfile.gettempdir(), "chain-review")

os.makedirs(OUTPUT_DIR, exist_ok=True)

PHASE = int(sys.argv[1]) if len(sys.argv) > 1 else 1

with open(OPCTX_PATH, "r", encoding="utf-8") as f:
    OPCTX = f.read()
...
REVIEW_QUESTION = """
You are reviewing ~/.claude/operator-context.md — the cold-instance brief loaded every session.
Your job: find what is wrong, outdated, or missing. Be specific. Cite line numbers or section names.

Investigate:
1. OPERATOR OVERRIDE section: dispatch method for large local models (MCP timeout vs. Invoke-RestMethod vs. Python streaming).
2. Deliberation chain sequence: does the document correctly reflect gemma -> qwen -> laguna -> granite -> nemotron?
3. Frontier model prohibition vs. governance gate circular dependency: how is this documented?
4. Timeout rules: does the document specify timeout >= num_ctx? Critical for cold instances.
5. think:False placement: top-level body key (not inside options) for qwen3.6 and nemotron-3-super.
6. Any other gaps, errors, or outdated content.
  ...
  "concerns": [
    {
      ...
      "recommended_fix": "what it should say instead",
      "investigation_task": "question for next agent"
    }
  ],
  "search_findings": "what live search revealed relevant to these questions",
...
PHASE1_AGENTS = [
    {
        "name": WORKSHOP_MODEL,
        "role": "workshop",
        "search_query": "Ollama MCP timeout CPU inference large models local governance document AI agent dispatch 2026",
        "think": None,
        "num_predict": 4096,
    },
    {
        ...
        "role": "deep-dive",
        "search_query": "Ollama API generate endpoint streaming Python requests dispatch vs MCP timeout large model CPU inference",
        "think": False,
        "num_predict": 4096,
    },
]
...
PHASE2_AGENTS = [
    {
        ...
        "role": "code-review",
        "search_query": "Ollama REST API PowerShell Invoke-RestMethod streaming dispatch method timeout configuration",
        "think": None,
        "num_predict": 3072,
    },
    {
        ...
        "search_query": "AI governance operator context document frontier model prohibition local quorum circular dependency policy",
        "think": None,
        "num_predict": 4096,
    },
    {
        ...
        "role": "synthesis",
        "search_query": "multi-agent deliberation governance document validation synthesis final verdict production 2026",
        "think": False,
        "num_predict": 32768,
        "num_ctx": 32768,
    },
...
def dispatch_agent(cfg, prior_verdicts, search_results, open_concerns, soft_notes=None):
    ...
    prompt = "\n".join([
        ...
        concern_block,
        soft_notes_block,
        f"\n[OPERATOR-CONTEXT.MD -- FULL CONTENT]\n{OPCTX}",
        f"\n[REVIEW QUESTION]\n{REVIEW_QUESTION}",
...
def main():
    print(f"\nCHAIN-REVIEW: Phase {PHASE}", flush=True)
    ...
    if PHASE == 1:
        ...
        print(f"  2. Read operator-context.md", flush=True)
        print(f"  3. Write synthesis to: {os.path.join(OUTPUT_DIR, 'sonnet-synthesis.txt')}", flush=True)
        print(f"  4. Run: python scripts/chain-review.py 2", flush=True)
