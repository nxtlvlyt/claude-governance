...
|---|---|
| **BIG-PROJECT READINESS GATE (operator word 2026-07-03 ~01:15: "I do have a big project for you once the chain can successfully e2e complete at least three non-visual missions and two visual missions 100% autonomously")** — counter starts 2026-07-03T01:15Z under the current engine stack; a mission COUNTS iff: fired by the daemon from AUTORUN (conductor may author/amend text BEFORE firing — that is routing, not rescue), completes DONE with result.json ok:true through the full chain incl. verdict panel PASS, ZERO mid-mission conductor intervention (no hand-applied steps, no conductor-direct completion, no RESOLVED-in-lieu-of-DONE), witness AFTER receipt present; VISUAL missions additionally need the headless-render receipt their VISUAL-QC bar demands. Tally reported EVERY beat in board format ("autonomy gate: N/3 non-visual · M/2 visual") with each counted mission named + receipt path. | → when 3/3 + 2/2: notify the operator LOUDLY (push + report headline) that the gate is MET and ask for the big project. A FAILED mission does not reset the tally (the bar is "can complete", not "never fails") — but a counted mission later found hollow (false-DONE) is struck with the receipt. |
