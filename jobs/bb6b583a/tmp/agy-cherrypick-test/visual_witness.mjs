// visual_witness.mjs — visual-regression witness via visionVerdict (agy Gemini 3.5 Flash
// workspace-read first, mistral-small3.2:24b@nxtbeast failover)
//
// Uses visionVerdict from ./ollama_vision_verdict.mjs to compare deployed-preview
// screenshots against the baseline screenshots captured by capture-visreg-baseline.mjs.
//
// HISTORY: this module originally replaced the agy path ("--print returns empty stdout",
// 2026-06-24) with Ollama Cloud's gemini-3-flash-preview; the NO-CLOUD ruling (2026-07-02)
// then killed the cloud lane and vision duty moved to local gemma → mistral. RE-POINTED
// 2026-07-08 (fork intake 3): agy print-mode vision WORKS via workspace file read (probed;
// Gemini 3.5 Flash seated as visual QC per operator pick, fork commit 57f5609) —
// visionVerdict tries that lane first and fails over to the local mistral seat, so this
// witness keeps working through either lane. The old OLLAMA_API_KEY gate is GONE: neither
// live lane (agy CLI, local nxtbeast) needs a cloud key, and keeping it would have
// fail-closed the whole witness on machines without a stale env var.
//
// Wired into orchestrate.mjs (applyVisualWitness) as ADVISORY ONLY — not a Phase-3 voting
// seat (orchestrate.mjs:264/466; the old "not yet wired" claim here was stale, corrected
// 2026-07-08).
//
...
import { visionVerdict } from './ollama_vision_verdict.mjs';

const HERE = path.dirname(import.meta.url.replace(/^file:\/+/, '')).replace(/\\/g, '/');
const BASELINE_DIR = path.join(HERE, 'qc-baseline');
// Model choice now lives in visionVerdict (agy label default + local failover seat). A
// caller-supplied opts.model still passes through; the old DEFAULT_MODEL constant was the
// forbidden Ollama Cloud slug ('gemini-3-flash-preview') and is gone — receipts label the
// default lanes explicitly instead.
const MODEL_LABEL_DEFAULT = 'visionVerdict-default (agy Gemini 3.5 Flash (Low) -> mistral-small3.2:24b@nxtbeast)';
...
// witnessVisualDiff — capture preview screenshots, dispatch visionVerdict (agy-first,
// local-ollama failover) to compare them against baselines, parse the verdict, return
// structured receipt.
//
...
export async function witnessVisualDiff(previewPathFn, opts = {}) {
  const t0 = Date.now();
  const model = opts.model || MODEL_LABEL_DEFAULT;
  ...
  // opts.model is only forwarded when the caller set one — visionVerdict owns lane-scoped
  // model defaults (an agy display label there, the local mistral seat on failover).
  const r = await visionVerdict(prompt, imagePaths, {
    ...(opts.model ? { model: opts.model } : {}),
    timeoutMs: opts.timeoutMs || 5 * 60 * 1000,
  });

  const responseText = sanitizeWitnessContent(r?.response).content;
  const ranModel = r?.model || model;   // receipt carries the lane that actually ran

  if (!r.ok) {
    return {
      ok: false, verdict: 'error', pages_compared: pairs.map(p => ({ ...p, diff_summary: '(vision verdict failed)', finding: null })),
      blocking_findings: [], elapsedMs: Date.now() - t0, model: ranModel,
      error: { kind: r.error || 'VISION_VERDICT_FAIL', detail: r.raw || '' },
      ...(r.failover ? { failover: r.failover } : {}),
    };
  }
