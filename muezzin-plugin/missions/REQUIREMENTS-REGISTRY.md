...
|---|---|---|---|---|
| A1 | POI/roadside info richness (oddities, roadsides had no useful info) | poi-tags, poi-services-nearby, poi-hover-preview-cards | **NOT-DONE** | 15 POI types are rich, but `roadside_oddity`/`charging`/`locker` have NO `popup()` branch → empty placeholder. The 3 enrichment missions are on feature branches, NOT in HEAD (poi-tags/poi-services retros falsely DONE; poi-hover failed plan). |
