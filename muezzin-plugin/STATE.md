...
**Struck so far (2026-07-04, this session): 12 of the 25 hunt items** — #1 (dotted-stem
mission sandbox collision, the hunt's TOP FIND, 7ae0153), #2 (local-lane TIMEOUT/NETWORK heal
asymmetry, 5cba9d5), #4 (heartbeat failure-class table, fe46e4a2), #5 (daemon
UNRESOLVED-as-RESOLVED \b inversion, 548635f), #6 (RESOLVED-LANDED stamp validation,
1b65287+f05d0e3), #9 (self-witness prompt truncation, 831dead), #10 (self-witness receipts
mislabeling ornith:9b as 'laguna', b13ff7c), #12 (no-verdict re-ask now allows a concern line,
5d8fc1e), #15 (fix-ledger requeue-once .some() bug, cb249ea), #17 (GAP-PRIORITY-HOLD namespace
widened beyond mt-*, 72a17f6), #19 (conduct-cycle divergence guard fail-open-on-git-error,
6c1363a), #20 (DEPLOY keystroke's --commit-dirty contradiction clarified, 959eb68).
...
**17 remain open** (4 top-level + 13 hunt items).
