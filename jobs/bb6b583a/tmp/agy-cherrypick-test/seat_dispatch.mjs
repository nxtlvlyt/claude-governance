...
//   - waterfall = [agy] -> [named/preferred Claude] -> [ollama-cloud for censused cloud tags,
//     restored 2026-07-07 in THIS FORK — see CLOUD_PROVIDER note] -> local + 3 ADAPTIVE HEALS
//     -> Claude tier  (parent plugin remains de-clouded per the NO-CLOUD ruling 2026-07-02)
...
import { dispatchAgy, agyAvailable, resolveAgyModel, isAgyModelLabel } from './agy_dispatch.mjs';
...
// PROVIDERS — FORK JURISDICTION NOTE (agy-muezzin step 2b, 2026-07-07): in the PARENT
// muezzin-plugin the operator's NO-CLOUD ruling (2026-07-02, operator-rulings.md) removed the
// ollama-cloud provider structurally on 2026-07-03 (last real cloud dispatch 2026-07-02T13:37Z,
// 429-refused; zero since, adversarially verified wf_526bff17). THIS file is the agy-muezzin
// FORK — its own git repo and jurisdiction, where Ollama Cloud IS permitted (spec: the
// agy-implementation-research plan §3, authored in the parent jurisdiction 2026-07-07). The
// restored cloud lane below follows that report's verified API surface:
//   - NATIVE /api/chat on https://ollama.com — NEVER /v1 (§3.2 trap: the OpenAI-compat shim
//     SILENTLY IGNORES `options`, so num_ctx/num_predict control REQUIRES the native endpoint;
//     current ollama.com/v1 availability is itself only HYPOTHESIS). Native response shape is
//     `message.content`, not `choices[0].message.content`.
//   - Authorization: Bearer $OLLAMA_API_KEY (primary; OLLAMA_CLOUD_API_KEY secondary — the two
//     keys this codebase already recognizes, §3.5). FAIL CLOSED with kind=MISSING_OLLAMA_API_KEY
//     when neither is set: never an unauthenticated dispatch.
//   - stream:false (native /api/chat streams by default, §3.1); bare tags on the direct call
//     (:cloud/-cloud suffixes are for the local-proxy convention and are stripped, §3.1).
// The parent plugin remains de-clouded; nothing in the parent imports this file.
const LOCAL_PROVIDER = { id: 'ollama-local', url: 'http://nxtbeast:11434/v1/chat/completions', envKeys: [] };
const CLOUD_PROVIDER = { id: 'ollama-cloud', url: 'https://ollama.com/api/chat', envKeys: ['OLLAMA_API_KEY', 'OLLAMA_CLOUD_API_KEY'] };

// OLLAMA CLOUD CATALOG (censused 2026-07-07, agy-implementation-research.md §3.4 — the full
// 20-entry ollama.com/search?c=cloud list; base names, matched against the tag's pre-':' part).
// Report caveats carried honestly: the census is [UNADJUDICATED] (WebFetch-summarizer sourced);
// qwen3-coder:480b + qwen3-coder-next, glm-5/glm-4.7, minimax-m2.1, gemini-3-flash-preview and
// others RETIRE 2026-07-15 per the docs' deprecation table — re-census after that date.
export const OLLAMA_CLOUD_CATALOG = new Set([
  'glm-5.2', 'kimi-k2.7-code', 'gemma4', 'qwen3.5', 'glm-5.1', 'minimax-m2.7',
  'nemotron-3-super', 'glm-5', 'minimax-m2.5', 'minimax-m3', 'kimi-k2.6',
  'deepseek-v4-flash', 'deepseek-v4-pro', 'kimi-k2.5', 'nemotron-3-ultra',
  'gpt-oss', 'qwen3-coder', 'glm-4.7', 'gemini-3-flash-preview', 'minimax-m2.1',
]);
// Tags that collide with the cloud catalog by BASE NAME but are receipted-LOCAL in this fork
// and must never route cloud: gemma4:31b is the local vision/architect seat carrying the
// MODEL_OPTIONS num_gpu:56 partial-offload overlay (gap #10 ARM 1) — a cloud dispatch would
// silently bypass that engineering. (Cloud gemma4 variants like gemma4:12b still qualify.)
const CLOUD_LOCAL_OVERRIDES = new Set(['gemma4:31b']);
// isCloudModelTag — PURE gate (mirrors routePrefersAgy's role for the agy lane): true only for
// a tag the 2026-07-07 census says ollama.com serves, that is NOT an agy display label, NOT a
// Claude family name, and NOT a receipted-local override. An explicit :cloud/-cloud suffix is
// an unambiguous cloud request regardless of catalog membership.
export function isCloudModelTag(model) {
  const m = String(model || '').trim();
  if (!m) return false;
  if (isAgyModelLabel(m)) return false;            // agy display labels are agy-only by definition
  if (recognizeClaudeModel(m)) return false;       // Claude names are Anthropic-only (never ollama)
  if (CLOUD_LOCAL_OVERRIDES.has(m)) return false;  // receipted-local collision (see above)
  if (/(?::cloud|-cloud)$/.test(m)) return true;   // explicit cloud-suffix convention (§3.1)
  return OLLAMA_CLOUD_CATALOG.has(m.split(':')[0]);
}
...
export function claudeFallbackFor(model, role) {
  // AGY DISPLAY LABELS (atv-1.S1 fix, 2026-07-08): a label seat's ONLY relief lane is the
  // Claude tier — ollama 400s "invalid model name" on labels (8 receipts, dispatch-heartbeat
  // 11:57:06-07), and the flat map below is ollama-name -> claude, so labels had NO mapping
  // ("no claude-tier mapping for this seat", atv-1.S1 result.json:9). Family-mapped: an
  // Opus-class label falls to opus; every other label falls to sonnet (the flat map's own
  // precedent for frontier verdict seats: deepseek-v4-pro -> sonnet).
  if (isAgyModelLabel(model)) return /\bOpus\b/.test(String(model)) ? 'opus' : 'sonnet';
...
// agy FIRST (before namedClaude and the local waterfall). Burns agy's separate quota
// (~5-hour refresh + weekly per-family pools — the old "4-hour rolling" claim was refuted,
// agy-implementation-research.md §2.4); spares the weekly direct-API Claude budget for the
// heaviest phase. OFF BY DEFAULT — existing waterfall behavior unchanged when flag not set.
//
...
function routePrefersAgy(model) {
  ...
  // agy DISPLAY-LABEL seats (agy-hybrid mode, 2026-07-07): a seat named with one of the 8
  // verified `agy models` labels is agy-only by definition (not an Ollama tag, not a
  // Claude-CLI name — it would 404 locally) and MUST route via the agy lane.
  if (isAgyModelLabel(model) && agyAvailable()) return true;
...
async function attemptAgy(body, seatOrModel, timeoutMs, cwd) {
  ...
  if (!r.ok) {
    // Carry the stderr TAIL on the error message (fork intake 5): the receipted agy quota shape
    // (`RESOURCE_EXHAUSTED (code 429): Individual quota reached. ... Resets in <duration>.`,
    // agy_dispatch.mjs header §2.4) rides stderr, and a bare NONZERO_EXIT detail ("agy exited
    // with code N") is invisible to quotaWallKind. The tail keeps the trailing `Resets in`
    // duration that separates a session wall from a weekly-class wall.
    const failTail = String(r.stderr || '').replace(/\s+/g, ' ').trim().slice(-300);
    throw new WaterfallError(r.error?.kind || 'AGY_FAILED', 'agy', agyModel,
      (r.error?.detail || `agy exited ${r.exitCode}`) + (failTail ? ` (stderr: ${failTail})` : ''));
  }
  // EMPTY CONTENT IS AN ERROR, NEVER A RESULT (P0-CORPUS law; parity with attemptClaude
  // below). The old placeholder string ('(empty stdout — agy planner-mode ...)') poisoned
  // wantVerdict seats: extractJson failed on it and burned the attempt
  // (agy-implementation-research.md §4.3.1). Throwing lets the waterfall fall through to
  // the next lane; executor deed-verification lives in the runner's execReceipt, not here.
  const content = r.stdout.trim();
  if (!content) {
    const errTail = String(r.stderr || '').replace(/\s+/g, ' ').trim().slice(-120);
    throw new WaterfallError('EMPTY_CONTENT', 'agy', agyModel,
      `agy returned empty stdout (planner-loop swallow; stderr: ${errTail || 'empty'})`);
  }
...
// FORK-LOCAL route file (jurisdiction fix 2026-07-07, second instance: commit 40da93b
// re-pointed seat_modes.mjs's ROUTE_FILE but MISSED this one — routePrefersAgy Gate 2 and
// routePrefersClaude were still reading the Claude-side state dir, so a prefer/standing_prefer
// written by the OTHER system could silently steer THIS fork's dispatch order. Same pattern
// as seat_modes.mjs: env override first, then the fork's own muezzin-route.json.)
const ROUTE_FILE = process.env.AGY_MUEZZIN_ROUTE || 'C:/Users/marka/agy-muezzin/muezzin-route.json';
...
// ------------------------------------------------------------------ QUOTA-WALL AWARENESS (fork intake 5)
// RECORD-ONLY + CONSERVATIVE SKIP. When the agy or cloud lane dies on a RECEIPTED quota-wall
// shape, the wall is recorded per model FAMILY (agy quota pools are per-family — agy_dispatch.mjs
// header §2.4: ~5-hour refresh cycle PLUS weekly per-model-family pools) and the family gets a
// bounded cooldown. While the cooldown is live, the lane is SKIPPED at entry with an hb
// `attempt-skip` line and dispatch falls through to the next lane — never a throw, and the
// local/Claude lanes are never consulted or affected. Only receipted shapes classify; everything
// else is null (a TIMEOUT is not a wall; an HTTP_500 is not a wall — conservative by design).
// Store: missions/_logs/quota-walls.json  {family: {kind, count, lastTs, cooldownUntil}}.
// Recording and consulting are BEST-EFFORT (try/catch): a corrupt store or unwritable path must
// never break dispatch — the store is a cache, not substrate. MUEZZIN_QUOTA_WALL_FILE env (read
// per call, same pattern as hb()'s MUEZZIN_HB_FILE) redirects the store for selftests;
// production leaves it unset.
const QUOTA_WALL_FILE_DEFAULT = join(dirname(fileURLToPath(import.meta.url)), 'missions', '_logs', 'quota-walls.json');
function quotaWallFile() { return process.env.MUEZZIN_QUOTA_WALL_FILE || QUOTA_WALL_FILE_DEFAULT; }

// parseResetHours — total hours from the agy quota message's trailing `Resets in <duration>`
// (receipted forms: 1h12m, 3h46m, 53h47m, 99h51m; a `d` component is accepted defensively).
// null = no parseable duration present.
function parseResetHours(text) {
  const m = /resets?\s+in\s*:?\s*(?:(\d+)\s*d)?\s*(?:(\d+)\s*h)?\s*(?:(\d+)\s*m)?/i.exec(String(text || ''));
  if (!m || (m[1] === undefined && m[2] === undefined && m[3] === undefined)) return null;
  return Number(m[1] || 0) * 24 + Number(m[2] || 0) + Number(m[3] || 0) / 60;
}

// quotaWallKind — classify a lane error (WaterfallError-shaped object or bare string) into
// '429' | 'session-wall' | 'weekly-wall' | null. ONLY the receipted shapes classify:
//   - `RESOURCE_EXHAUSTED (code 429): Individual quota reached. ... Resets in <duration>.`
//     (agy_dispatch.mjs header, verified 2026-07-07) — the reset duration separates a session
//     wall (receipted 1h12m-3h46m; the ~5h refresh cycle) from a weekly-class wall (receipted
//     53h47m / 99h51m): >6h reads weekly. Unparseable duration defaults to session-wall — the
//     SHORTER cooldown is the conservative error.
//   - "weekly usage limit" (the cloud circuit-breaker receipt 2026-07-02: 92 repeats over 31h
//     against "you have reached your weekly usage limit") -> weekly-wall.
//   - the cloud lane's HTTP_429 WaterfallError kind / a bare `429: <body>` message -> '429'
//     (a rate spike, not a proven wall — shortest cooldown).
// Anything else -> null (not a quota wall; the caller records nothing).
export function quotaWallKind(err) {
  if (err == null) return null;
  const kind = (typeof err === 'object' && err.kind) ? String(err.kind) : '';
  const msg = typeof err === 'string' ? err : String(err.message ?? '');
  if (!kind && !msg) return null;
  const text = `${kind} ${msg}`;
  if (/weekly usage limit/i.test(text)) return 'weekly-wall';
  if (/RESOURCE_EXHAUSTED/i.test(text) || /individual quota reached/i.test(text)) {
    const hours = parseResetHours(text);
    if (hours != null && hours > 6) return 'weekly-wall';
    return 'session-wall';
  }
  if (kind === 'HTTP_429' || /^429\b/.test(msg.trim())) return '429';
  return null;
}

// familyOf — model name -> quota-family key. agy quota pools are per model FAMILY (header §2.4);
// cloud walls are conservatively pooled the same way. The 8 verified agy display labels and the
// censused cloud tags/slugs both map; a receipted-local override (gemma4:31b) maps to 'local'
// (it never rides a quota'd lane); anything unknown is its OWN family (the model string itself)
// so an unknown model's wall can never shadow — or be shadowed by — a known family's pool.
const FAMILY_PREFIXES = ['gpt-oss', 'kimi', 'glm', 'deepseek', 'minimax', 'qwen', 'nemotron'];
export function familyOf(model) {
  const m = String(model || '').trim();
  if (!m) return m;
  if (CLOUD_LOCAL_OVERRIDES.has(m)) return 'local';
  if (isAgyModelLabel(m)) {
    if (/^gemini\b.*\bflash\b/i.test(m)) return 'gemini-flash';
    if (/^gemini\b.*\bpro\b/i.test(m)) return 'gemini-pro';
    if (/^claude\b/i.test(m)) return 'claude-agy';   // Claude VIA AGY — a separate pool from direct-Anthropic Claude (whose lanes never consult walls)
    if (/^gpt-oss\b/i.test(m)) return 'gpt-oss';
  }
  const base = m.toLowerCase().replace(/(?::cloud|-cloud)$/, '').split(':')[0];
  if (base.startsWith('gemini')) return /(pro|ultra)/.test(base) ? 'gemini-pro' : 'gemini-flash';
  for (const fam of FAMILY_PREFIXES) if (base.startsWith(fam)) return fam;
  return m;
}

// wallCooldownMs — bounded, kind-proportional cooldowns. 429/session-wall: 30min (the session
// cycle refreshes ~5-hourly and observed session resets ran 1h12m-3h46m — a 30min re-probe is
// cheap and self-corrects a misread). weekly-wall: 6h (receipted weekly resets ran 53-99h, but
// 6h keeps the skip CONSERVATIVE — the lane re-probes a few times before a genuine weekly reset
// rather than self-blinding for days on one classification). Unknown kind: 0 (no cooldown invented).
export function wallCooldownMs(kind) {
  if (kind === 'weekly-wall') return 6 * 60 * 60 * 1000;
  if (kind === '429' || kind === 'session-wall') return 30 * 60 * 1000;
  return 0;
}

// recordQuotaWall — best-effort upsert of {family: {kind, count, lastTs, cooldownUntil}} into
// the store. count accumulates across records (forensics: how often does this family wall?);
// kind/cooldownUntil reflect the LATEST wall (a live cooldown skips the lane, so a newer record
// can only land after the prior cooldown expired — overwrite is monotone in practice). A corrupt
// store is discarded and re-seeded (cache, not substrate). Returns the entry, or null on any
// failure — RECORDING MUST NEVER BREAK DISPATCH.
export function recordQuotaWall(family, kind, now = Date.now()) {
  try {
    const file = quotaWallFile();
    let db = {};
    try { db = JSON.parse(readFileSync(file, 'utf8')); } catch { db = {}; }
    if (!db || typeof db !== 'object' || Array.isArray(db)) db = {};
    const count = ((db[family] && Number(db[family].count)) || 0) + 1;
    const entry = {
      kind,
      count,
      lastTs: new Date(now).toISOString(),
      cooldownUntil: new Date(now + wallCooldownMs(kind)).toISOString(),
    };
    db[family] = entry;
    mkdirSync(dirname(file), { recursive: true });
    writeFileSync(file, JSON.stringify(db, null, 2));
    hb(`quota-wall-record family=${family} kind=${kind} count=${count} until=${entry.cooldownUntil}`);
    return entry;
  } catch { return null; }   // best-effort by contract
}

// quotaWallHold — the consult half: returns the family's entry while its cooldownUntil is in
// the future, else null (expired / unknown / unreadable store all read "lane free to run" —
// CONSULTING MUST NEVER BREAK DISPATCH, and a broken store fails OPEN into normal dispatch).
export function quotaWallHold(family, now = Date.now()) {
  try {
    const db = JSON.parse(readFileSync(quotaWallFile(), 'utf8'));
    const e = db?.[family];
    if (!e || !e.cooldownUntil) return null;
    const until = Date.parse(e.cooldownUntil);
    return (Number.isFinite(until) && until > now) ? e : null;
  } catch { return null; }
}
...
// ------------------------------------------------------------------ OLLAMA-CLOUD LANE (fork)
// toNativeCloudBody — translate the engine's OpenAI-shape body to the NATIVE /api/chat shape
// (agy-implementation-research.md §3.1/§3.2). The native endpoint's `options` object is the
// ONLY way to set num_ctx/num_predict (the /v1 compat shim silently drops `options` — the
// receipted trap, ollama_vision_verdict.mjs:45-52); top-level OpenAI fields are mapped in,
// with any explicit caller-set options winning. Direct ollama.com calls use BARE tags, so the
// local-proxy :cloud/-cloud suffixes are stripped (same normalization healDispatch's HTTP_404
// branch applies reactively).
function toNativeCloudBody(body) {
  const model = String(body.model || '').replace(/:cloud$/, '').replace(/-cloud$/, '');
  const options = { ...(body.options || {}) };
  if (body.max_tokens != null && options.num_predict == null) options.num_predict = body.max_tokens;
  if (body.temperature != null && options.temperature == null) options.temperature = body.temperature;
  if (body.top_p != null && options.top_p == null) options.top_p = body.top_p;
  return {
    model,
    messages: body.messages,
    stream: false,   // /api/chat streams by DEFAULT (§3.1) — one complete response, like every other lane
    ...(body.think !== undefined ? { think: body.think } : {}),
    ...(Object.keys(options).length ? { options } : {}),
  };
}

// attemptOllamaCloud — ONE native /api/chat attempt against ollama.com. Fails CLOSED with
// kind=MISSING_OLLAMA_API_KEY before any network when no key env is set (never an
// unauthenticated dispatch). AbortSignal-bounded like the local lane (no subprocess, so no
// tree-kill needed). SINGLE-SHOT BY DESIGN in step 2b: no SearXNG tool loop on this lane yet —
// the native tool-call response shape differs from OpenAI's (function.arguments is an object,
// not a JSON string), so tool-loop parity is a separate receipted step, not a silent bolt-on.
// Exported for the offline selftest (mocked fetch, both polarities).
export async function attemptOllamaCloud(rawBody, timeoutMs) {
  const apiKey = keyFor(CLOUD_PROVIDER);
  if (!apiKey) {
    throw new WaterfallError('MISSING_OLLAMA_API_KEY', CLOUD_PROVIDER.id, rawBody?.model,
      `Missing ${CLOUD_PROVIDER.envKeys.join('/')} — cloud lane fails CLOSED (no unauthenticated dispatch); set the key or leave cloud tags unseated`);
  }
  const native = toNativeCloudBody(rawBody);
  const controller = new AbortController();
  const tid = setTimeout(() => controller.abort(), timeoutMs);
  let res;
  try {
    res = await fetch(CLOUD_PROVIDER.url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
    ...
    clearTimeout(tid);
    if (e.name === 'AbortError') throw new WaterfallError('TIMEOUT', CLOUD_PROVIDER.id, native.model, `timeout ${timeoutMs}ms`);
    throw new WaterfallError('NETWORK', CLOUD_PROVIDER.id, native.model, e.message);
  }
  clearTimeout(tid);
  if (!res.ok) {
    let t = ''; try { t = await res.text(); } catch { }
    throw new WaterfallError(`HTTP_${res.status}`, CLOUD_PROVIDER.id, native.model, `${res.status}: ${t.slice(0, 200)}`);
  }
  let data; try { data = await res.json(); } catch (e) { throw new WaterfallError('API_ERROR', CLOUD_PROVIDER.id, native.model, e.message); }
  if (data.error) throw new WaterfallError('API_ERROR', CLOUD_PROVIDER.id, native.model, typeof data.error === 'string' ? data.error : JSON.stringify(data.error));
  const m = data.message;   // NATIVE shape: message.content, not choices[0].message.content (§3.2)
  if (!m) throw new WaterfallError('API_ERROR', CLOUD_PROVIDER.id, native.model, 'no message in native response');
  const content = String(m.content || '').trim();
  if (!content) {
    // EMPTY CONTENT IS AN ERROR, NEVER A RESULT (P0-CORPUS law — parity with every other lane).
    const thinking = String(m.thinking || '');
    throw new WaterfallError(thinking ? 'EMPTY_CONTENT_THINKING' : 'EMPTY_CONTENT', CLOUD_PROVIDER.id, native.model,
      thinking ? `content empty; thinking consumed budget (${thinking.length} chars; head: ${thinking.slice(0, 200)})` : 'content empty, no thinking returned');
  }
  return { content, toolTrace: [], usage: { prompt: data.prompt_eval_count || 0, completion: data.eval_count || 0 } };
}

// dispatchOllamaCloudLane — the lane runner: heal parity with the local lane's ONE-SHOT
// TIMEOUT/NETWORK heals (the mt-b1-local-heal hunt-2 pattern above: one extend-and-retry on
// TIMEOUT, one 600ms-delay retry on NETWORK/HTTP_5xx, then THROW). The waterfall catches the
// throw and falls through to the local lane — same safe rail as the agy lane. Heartbeat lines
// carry provider=ollama-cloud in the standard attempt-start/ok/fail format the STUCK-TASK
// decision reads. Exported for the offline selftest.
export async function dispatchOllamaCloudLane(baseBody, { remaining = () => FETCH_TIMEOUT_MS } = {}) {
  let timeoutHealed = false;   // one extend-and-retry, then fail over (healDispatch TIMEOUT parity)
  let networkHealed = false;   // one 600ms-delay retry on NETWORK/5xx, then fail over
  let heal = 0;
  let cloudTimeoutMs = FETCH_TIMEOUT_MS;
  for (;;) {
    hb(`attempt-start provider=${CLOUD_PROVIDER.id} model=${baseBody.model} heal=${heal} timeout=${Math.min(cloudTimeoutMs, remaining())}ms`);
    const t0 = Date.now();
    try {
      const out = await attemptOllamaCloud(baseBody, Math.min(cloudTimeoutMs, remaining()));
      hb(`attempt-ok provider=${CLOUD_PROVIDER.id} model=${baseBody.model} heal=${heal} ms=${Date.now() - t0} chars=${out.content.length} tokens=${out.usage?.prompt || 0}+${out.usage?.completion || 0}`);
      return { ...out, provider: CLOUD_PROVIDER.id, heals: heal };
    } catch (e) {
      hb(`attempt-fail provider=${CLOUD_PROVIDER.id} model=${baseBody.model} heal=${heal} ms=${Date.now() - t0} kind=${e.kind || e.name}: ${String(e.message).slice(0, 120)}`);
      if (e.kind === 'TIMEOUT' && !timeoutHealed && remaining() > 60000) {
        timeoutHealed = true; heal++;
        cloudTimeoutMs = Math.min(cloudTimeoutMs * 2, 600000);
        continue;
      }
      const isNetworkOr5xx = e.kind === 'NETWORK' || /^HTTP_5/.test(e.kind || '');
      if (isNetworkOr5xx && !networkHealed && remaining() > 60000) {
        networkHealed = true; heal++;
        hb('heal-wait 600ms');
        await new Promise((r) => setTimeout(r, 600));
        continue;
      }
      throw e;   // lane exhausted (incl. MISSING_OLLAMA_API_KEY fail-closed) — caller falls through to local
    }
  }
}

// local (adaptive heals) -> Claude tier. Returns { content, provider, heals }.
// De-clouded 2026-07-03 (operator NO-CLOUD ruling); CLOUD RESTORED 2026-07-07 in this fork as
// a dedicated lane ABOVE the local lane for censused cloud tags (see dispatchWithWaterfall);
// the local lane keeps the adaptive-heal loop it inherited.
...
export async function dispatchWithWaterfall(baseBody, { cwd, localOnly = false, role = undefined } = {}) {
  ...
  // AGY-LABEL EXCLUSION (atv-1.S1 fix, 2026-07-08): an agy display label is a guaranteed
  // ollama 400 ("invalid model name") — no localOnly flag can make it a local model. The
  // label wins over the flag and rides the label lanes below (agy first, Claude relief);
  // this makes the label check authoritative at BOTH waterfall entrances.
  if (localOnly && !isAgyModelLabel(baseBody.model)) {
  ...
  // -- AGY LANE: when USE_AGY_EXECUTOR=true OR route prefer:"agy" OR the seat model is an
  // agy display label, try agy first. Burns agy's separate quota (~5h refresh + weekly
  // per-family pools) instead of the weekly Claude budget. On any failure (binary missing,
  // timeout, non-zero exit, empty stdout, etc) falls through to the existing
  // namedClaude/local waterfall — same safe rail as the Claude tier.
  if (routePrefersAgy(baseBody.model) && remaining() > 30000) {
    const agyModel = resolveAgyModel(baseBody.model);
    // QUOTA-WALL CONSULT (fork intake 5): a family in live cooldown SKIPS the lane without
    // burning it — never a throw, and preferTried stays false (the lane never ran, so the
    // post-local Claude tier stays available; a skip must not also cost the seat its relief
    // tier). Consult keys on the RESOLVED agy model (the display label), matching what the
    // record half writes on a terminal agy-lane failure below.
    const agyFamily = familyOf(agyModel);
    const agyHold = quotaWallHold(agyFamily);
    if (agyHold) {
      hb(`attempt-skip provider=agy model=${agyModel} kind=QUOTA_WALL family=${agyFamily} until=${agyHold.cooldownUntil}`);
      ...
      try {
        ...
        // QUOTA-WALL RECORD (fork intake 5): this catch IS the agy lane's terminal error path
        // (the lane has no heals — one attempt, then fall-through). Best-effort by contract:
        // recordQuotaWall never throws, so recording can never break the fall-through.
        const wall = quotaWallKind(e);
        if (wall) recordQuotaWall(agyFamily, wall);
        lastErr = e;   // atv-1.S1 fix: the label gate below names the agy failure in its receipt
  ...
  // ---- AGY-LABEL AUTHORITATIVE GATE (atv-1.S1 receipts, 2026-07-08): a display label
  // ("GPT-OSS 120B (Medium)", "Gemini 3.5 Flash (Medium)", ...) is an AGY-ONLY name. Before
  // this gate, when the agy lane above failed (PROMPT_TOO_LONG argv cap, quota wall, binary
  // absent) the label FELL THROUGH into the local lane — ollama 400'd "invalid model name" on
  // every heal (8 receipts, dispatch-heartbeat 11:57:06-07), the post-local Claude tier was
  // suppressed by preferTried AND had no label mapping, so the seat dispatch-FAILED and the
  // verdict panel BLOCKed the mission. The label decides the lane set AT THE BOUNDARY: agy
  // first (above), the Claude tier as the ONLY relief (a failed agy attempt burned agy quota,
  // never Claude — the anti-double-charge suppression does not apply to labels), and the
  // ollama lanes NEVER (local is a guaranteed 400; cloud is already excluded by isCloudModelTag).
  if (isAgyModelLabel(baseBody.model)) {
    const relief = (process.env.MUEZZIN_CLAUDE_TIER === 'off') ? null : claudeFallbackFor(baseBody.model, role);
    if (relief && remaining() > 30000) {
      hb(`attempt-start provider=claude-${relief} (AGY-LABEL relief — ollama lanes skipped by design) timeout=${Math.min(CLAUDE_TIMEOUT_MS, remaining())}ms`);
      const tl = Date.now();
      try {
        const out = await attemptClaude(baseBody, relief, Math.min(CLAUDE_TIMEOUT_MS, remaining()), cwd);
        hb(`attempt-ok provider=claude-${relief} (agy-label relief) ms=${Date.now() - tl} chars=${out.content.length}`);
        return { ...out, provider: `claude-${relief}`, heals: 0, localError: lastErr?.message };
      } catch (e) {
        hb(`attempt-fail provider=claude-${relief} (agy-label relief) ms=${Date.now() - tl} kind=${e.kind || e.name}: ${String(e.message).slice(0, 120)}`);
        throw new WaterfallError('ALL_FAILED', 'waterfall', baseBody.model,
          `agy: ${lastErr?.message || 'lane unavailable (binary absent / quota hold / budget)'}; claude-relief: ${e.message} — agy display label never dispatches to ollama`);
      }
    }
    throw new WaterfallError('ALL_FAILED', 'waterfall', baseBody.model,
      `agy: ${lastErr?.message || 'lane unavailable (binary absent / quota hold / budget)'}; ${relief ? 'budget exhausted before claude relief' : 'claude tier off — no relief lane'} — agy display label never dispatches to ollama`);
  }
  ...
  // -- OLLAMA-CLOUD LANE (restored 2026-07-07, fork jurisdiction — see CLOUD_PROVIDER note):
  // a censused cloud tag (isCloudModelTag: catalog match, not an agy label, not a Claude name,
  // not a receipted-local override) dispatches to ollama.com's native /api/chat BEFORE the
  // local lane — these tags previously fell through to a guaranteed-404 local dispatch. Sits
  // AFTER the agy + named/preferred Claude lanes so operator dials (route windows,
  // standing_prefer, USE_AGY_EXECUTOR) keep precedence; on ANY lane failure (including the
  // fail-closed keyless throw) falls through to the local lane + Claude tier — the same safe
  // rail as the agy lane. Does NOT set preferTried: cloud is not a Claude-budget lane.
  if (isCloudModelTag(baseBody.model) && remaining() > 30000) {
    // QUOTA-WALL CONSULT (fork intake 5): a family in live cooldown SKIPS the cloud lane and
    // falls straight through to the local lane + Claude tier (the lane's existing safe rail).
    // Never throws; lastErr stays untouched (a skip is not a failure).
    const cloudFamily = familyOf(baseBody.model);
    const cloudHold = quotaWallHold(cloudFamily);
    if (cloudHold) {
      hb(`attempt-skip provider=ollama-cloud model=${baseBody.model} kind=QUOTA_WALL family=${cloudFamily} until=${cloudHold.cooldownUntil}`);
    } else {
      try {
        return await dispatchOllamaCloudLane(baseBody, { remaining });
      } catch (e) {
        lastErr = e;
        // QUOTA-WALL RECORD (fork intake 5): the lane's terminal throw (HTTP_429 is unhealed
        // inside the lane by design, so a 429 lands here on the first attempt). Best-effort:
        // recordQuotaWall never throws, so recording can never break the fall-through.
        const wall = quotaWallKind(e);
        if (wall) recordQuotaWall(cloudFamily, wall);
        /* fall through to the local lane — heartbeat already carries the attempt-fail receipt */
      }
    }
  }
  // -- LOCAL LANE (the only OTHER ollama lane; parent's NO-CLOUD ruling 2026-07-02 removed the
  // old cloud lane): up to MAX_HEALS
...
if (process.argv[1]?.endsWith('seat_dispatch.mjs') && process.argv.includes('--selftest')) {
  ...
  process.env.MUEZZIN_QUOTA_WALL_FILE = join(tmpdir(), 'muezzin-selftest-quota-walls.json');   // fork intake 5: wall fixtures must NEVER touch the production store the live lanes consult
  try { unlinkSync(process.env.MUEZZIN_QUOTA_WALL_FILE); } catch { /* stale walls from a PRIOR run would falsely skip the cloud-lane tests below */ }
  let pass = 0, fail = 0;
  ...
  // 13. OLLAMA-CLOUD LANE (agy-muezzin step 2b, 2026-07-07 — fork jurisdiction permits cloud).
  //     Everything MOCKED: zero live ollama.com calls; the real key (if any) is saved aside as
  //     an opaque handle and restored — never read into output, never printed.
  // 13a. PURE GATE isCloudModelTag — both polarities against the 2026-07-07 census.
  check('isCloudModelTag: glm-5.2 (censused cloud) -> true', isCloudModelTag('glm-5.2'), true);
  check('isCloudModelTag: deepseek-v4-pro -> true', isCloudModelTag('deepseek-v4-pro'), true);
  check('isCloudModelTag: minimax-m3 -> true', isCloudModelTag('minimax-m3'), true);
  check('isCloudModelTag: kimi-k2.7-code -> true (agy-hybrid executor seat, plan-named cloud)', isCloudModelTag('kimi-k2.7-code'), true);
  check('isCloudModelTag: nemotron-3-super:latest -> true (base-name catalog match)', isCloudModelTag('nemotron-3-super:latest'), true);
  check('isCloudModelTag: explicit :cloud suffix -> true (glm-5.2:cloud)', isCloudModelTag('glm-5.2:cloud'), true);
  check('isCloudModelTag: explicit -cloud suffix -> true (gpt-oss:120b-cloud)', isCloudModelTag('gpt-oss:120b-cloud'), true);
  check('isCloudModelTag: local tag qwen3.6:27b -> false', isCloudModelTag('qwen3.6:27b'), false);
  check('isCloudModelTag: local tag north-mini-code-toolcall -> false', isCloudModelTag('north-mini-code-toolcall'), false);
  check('isCloudModelTag: local witness laguna-xs-2.1:q8_0 -> false', isCloudModelTag('laguna-xs-2.1:q8_0'), false);
  check('isCloudModelTag: gemma4:31b -> false (receipted-local override: num_gpu:56 vision seat)', isCloudModelTag('gemma4:31b'), false);
  check('isCloudModelTag: gemma4:12b -> true (cloud variant, no local receipt)', isCloudModelTag('gemma4:12b'), true);
  check("isCloudModelTag: agy label 'Gemini 3.5 Flash (Medium)' -> false (agy-only by definition)", isCloudModelTag('Gemini 3.5 Flash (Medium)'), false);
  check("isCloudModelTag: agy label 'GPT-OSS 120B (Medium)' -> false EVEN THOUGH gpt-oss is censused", isCloudModelTag('GPT-OSS 120B (Medium)'), false);
  check('isCloudModelTag: Claude name sonnet -> false (Anthropic-only)', isCloudModelTag('sonnet'), false);
  check('isCloudModelTag: claude-sonnet-5 -> false', isCloudModelTag('claude-sonnet-5'), false);
  check('isCloudModelTag: empty/null -> false', isCloudModelTag('') || isCloudModelTag(null), false);

  await (async () => {
    const realFetch = globalThis.fetch;
    const savedKey = process.env.OLLAMA_API_KEY;         // opaque save/restore handle — NEVER logged
    const savedKey2 = process.env.OLLAMA_CLOUD_API_KEY;  // opaque save/restore handle — NEVER logged
    const restoreEnv = () => {
      globalThis.fetch = realFetch;
      if (savedKey === undefined) delete process.env.OLLAMA_API_KEY; else process.env.OLLAMA_API_KEY = savedKey;
      if (savedKey2 === undefined) delete process.env.OLLAMA_CLOUD_API_KEY; else process.env.OLLAMA_CLOUD_API_KEY = savedKey2;
    };
    try {
      // 13b. CLOUD TAG ROUTES TO THE CLOUD LANE: native endpoint, Bearer from env, stream:false,
      //      options passthrough (num_ctx) + max_tokens -> num_predict translation, native
      //      message.content parsing, usage from native eval counts. Model deliberately UNMAPPED
      //      in CLAUDE_SEAT_MAP/AGY_EXECUTOR_SEATS so no operator dial (route file) can divert it.
      process.env.OLLAMA_API_KEY = 'sk-selftest-fake-key';
      delete process.env.OLLAMA_CLOUD_API_KEY;
      const cloudUrls = [], cloudBodies = [], cloudAuth = [];
      let otherUrls = [];
      globalThis.fetch = async (url, opts) => {
        const u = String(url);
        if (u.includes('ollama.com')) {
          cloudUrls.push(u);
          cloudAuth.push(opts?.headers?.Authorization || '');
          try { cloudBodies.push(JSON.parse(opts.body)); } catch { cloudBodies.push({}); }
          return { ok: true, status: 200, async json() { return { message: { content: 'cloud says hi' }, prompt_eval_count: 5, eval_count: 7 }; }, async text() { return ''; } };
        }
        otherUrls.push(u);   // any non-cloud fetch on a cloud tag is a routing bug — bounded 404
        return { ok: false, status: 404, async text() { return 'unexpected non-cloud fetch (test stub)'; } };
      };
      let r1;
      try { r1 = await dispatchWithWaterfall({ model: 'minimax-m2.7', max_tokens: 4096, think: false, options: { num_ctx: 16384 }, messages: [{ role: 'user', content: 'x' }] }); }
      catch (e) { r1 = { provider: `threw:${e.kind}`, content: '' }; }
      check('cloud tag minimax-m2.7 resolves on provider=ollama-cloud', r1.provider, 'ollama-cloud');
      check('cloud lane hit the NATIVE /api/chat endpoint exactly once (never /v1 — options trap)', cloudUrls.length === 1 && cloudUrls[0] === 'https://ollama.com/api/chat', true);
      check('cloud lane never touched a non-cloud endpoint', otherUrls.length, 0);
      check('cloud lane sent Bearer auth from env OLLAMA_API_KEY (fake test key)', cloudAuth[0], 'Bearer sk-selftest-fake-key');
      check('cloud lane sent stream:false (native default is streaming)', cloudBodies[0]?.stream, false);
      check('cloud lane passed options.num_ctx through NATIVELY', cloudBodies[0]?.options?.num_ctx, 16384);
      check('cloud lane translated max_tokens -> options.num_predict', cloudBodies[0]?.options?.num_predict, 4096);
      check('cloud lane parsed NATIVE message.content (not choices[0])', r1.content, 'cloud says hi');
      check('cloud lane mapped usage from native eval counts', r1.usage?.prompt === 5 && r1.usage?.completion === 7, true);
      check('cloud lane zero heals on clean success', r1.heals, 0);
      // suffix strip on the DIRECT call (bare-tag convention §3.1): lane-direct, same mock.
      await dispatchOllamaCloudLane({ model: 'gpt-oss:120b-cloud', messages: [{ role: 'user', content: 'x' }] });
      check('cloud lane strips -cloud suffix for the direct ollama.com call (bare tag)', cloudBodies.at(-1)?.model, 'gpt-oss:120b');

      // 13c. LOCAL TAG NEVER ROUTES CLOUD (full waterfall, not localOnly): qwen3.6:27b is
      //      unmapped everywhere -> deterministic local lane; assert ZERO ollama.com fetches.
      const urlsSeen = [];
      globalThis.fetch = async (url, opts) => {
        urlsSeen.push(String(url));
        const parsed = (() => { try { return JSON.parse(opts?.body || '{}'); } catch { return {}; } })();
      ...
      let r2;
      try { r2 = await dispatchWithWaterfall({ model: 'qwen3.6:27b', messages: [{ role: 'user', content: 'x' }] }); }
      catch (e) { r2 = { provider: `threw:${e.kind}` }; }
      check('local tag qwen3.6:27b: ZERO requests to ollama.com (full waterfall)', urlsSeen.filter((u) => u.includes('ollama.com')).length, 0);
      check('local tag qwen3.6:27b: resolves on provider=ollama-local', r2.provider, 'ollama-local');

      // 13d. MISSING KEY FAILS CLOSED with the NAMED error, before any network.
      delete process.env.OLLAMA_API_KEY;
      delete process.env.OLLAMA_CLOUD_API_KEY;
      let keylessFetches = 0;
      globalThis.fetch = async () => { keylessFetches++; return { ok: false, status: 500, async text() { return 'should never be reached'; } }; };
      let keylessKind = '';
      try { await attemptOllamaCloud({ model: 'minimax-m2.7', messages: [{ role: 'user', content: 'x' }] }, 5000); }
      catch (e) { keylessKind = e.kind; }
      check('keyless attemptOllamaCloud throws kind=MISSING_OLLAMA_API_KEY (named, fail-closed)', keylessKind, 'MISSING_OLLAMA_API_KEY');
      check('keyless: ZERO fetches issued (fails closed BEFORE any network)', keylessFetches, 0);
      // waterfall level: the keyless cloud lane falls through to the local safe rail, still zero cloud fetches.
      const kwUrls = [];
      globalThis.fetch = async (url, opts) => {
        kwUrls.push(String(url));
        const parsed = (() => { try { return JSON.parse(opts?.body || '{}'); } catch { return {}; } })();
        return { ok: true, status: 200, async json() { return { choices: [{ message: { content: `ok model=${parsed.model}` } }], usage: { prompt_tokens: 1, completion_tokens: 1 } }; }, async text() { return 'ok'; } };
      };
      let r3;
      try { r3 = await dispatchWithWaterfall({ model: 'minimax-m2.7', messages: [{ role: 'user', content: 'x' }] }); }
      catch (e) { r3 = { provider: `threw:${e.kind}` }; }
      check('keyless waterfall: zero ollama.com fetches (fail-closed, no unauthenticated call)', kwUrls.filter((u) => u.includes('ollama.com')).length, 0);
      check('keyless waterfall: falls through to provider=ollama-local (safe rail)', r3.provider, 'ollama-local');

      // 13e. HEAL PARITY — 5xx heals ONCE (600ms-delay retry) then THROWS out of the lane.
      process.env.OLLAMA_API_KEY = 'sk-selftest-fake-key';
      let fiveXxCalls = 0;
      globalThis.fetch = async () => { fiveXxCalls++; return { ok: false, status: 500, async text() { return 'upstream boom'; } }; };
      let fiveXxOut;
      try { await dispatchOllamaCloudLane({ model: 'minimax-m2.7', messages: [{ role: 'user', content: 'x' }] }); fiveXxOut = { kind: 'resolved' }; }
      catch (e) { fiveXxOut = { kind: 'threw', k: e.kind }; }
      check('cloud 5xx heal: exactly 2 attempts (1 initial + 1 one-shot heal)', fiveXxCalls, 2);
      check('cloud 5xx heal: lane THROWS HTTP_500 after the second 5xx (waterfall then falls through)', fiveXxOut.kind === 'threw' && fiveXxOut.k === 'HTTP_500', true);

      // 13f. HEAL PARITY — TIMEOUT heals ONCE (extend-and-retry) then THROWS.
      let toCalls = 0;
      globalThis.fetch = async () => { toCalls++; const err = new Error('simulated timeout'); err.name = 'AbortError'; throw err; };
      let toOut;
      try { await dispatchOllamaCloudLane({ model: 'minimax-m2.7', messages: [{ role: 'user', content: 'x' }] }); toOut = { kind: 'resolved' }; }
      catch (e) { toOut = { kind: 'threw', k: e.kind }; }
      check('cloud TIMEOUT heal: exactly 2 attempts (1 initial + 1 extend-retry)', toCalls, 2);
      check('cloud TIMEOUT heal: lane THROWS TIMEOUT after the second timeout', toOut.kind === 'threw' && toOut.k === 'TIMEOUT', true);
    } finally {
      restoreEnv();
    }
  })();

  // 14. QUOTA-WALL AWARENESS (fork intake 5): record-only + conservative family cooldown skip,
  //     agy + cloud lanes only. Receipted shapes both polarities; ZERO live calls.
  // 14a. quotaWallKind — each receipted shape classifies; non-quota errors -> null.
  check("quotaWallKind: agy session wall 'Resets in 3h46m' -> session-wall",
    quotaWallKind('RESOURCE_EXHAUSTED (code 429): Individual quota reached. Please try again later. Resets in 3h46m.'), 'session-wall');
  check("quotaWallKind: agy session wall 'Resets in 1h12m' -> session-wall",
    quotaWallKind('RESOURCE_EXHAUSTED (code 429): Individual quota reached. Resets in 1h12m.'), 'session-wall');
  check("quotaWallKind: agy weekly-class wall 'Resets in 53h47m' -> weekly-wall",
    quotaWallKind('RESOURCE_EXHAUSTED (code 429): Individual quota reached. Resets in 53h47m.'), 'weekly-wall');
  check("quotaWallKind: agy weekly-class wall 'Resets in 99h51m' -> weekly-wall",
    quotaWallKind('RESOURCE_EXHAUSTED (code 429): Individual quota reached. Resets in 99h51m.'), 'weekly-wall');
  check('quotaWallKind: agy quota with UNPARSEABLE reset -> session-wall (shorter cooldown = conservative default)',
    quotaWallKind('RESOURCE_EXHAUSTED (code 429): Individual quota reached.'), 'session-wall');
  check('quotaWallKind: agy NONZERO_EXIT carrying the quota text in the stderr tail -> classified',
    quotaWallKind({ kind: 'NONZERO_EXIT', message: 'agy exited with code 1 (stderr: RESOURCE_EXHAUSTED (code 429): Individual quota reached. Resets in 2h3m.)' }), 'session-wall');
  check("quotaWallKind: cloud weekly text on HTTP_429 -> weekly-wall (the 92x/31h circuit-breaker shape)",
    quotaWallKind({ kind: 'HTTP_429', message: '429: you have reached your weekly usage limit' }), 'weekly-wall');
  check("quotaWallKind: cloud HTTP_429 kind, generic body -> '429'",
    quotaWallKind({ kind: 'HTTP_429', message: '429: rate limited' }), '429');
  check("quotaWallKind: bare '429: <body>' string -> '429'", quotaWallKind('429: slow down'), '429');
  check('quotaWallKind: TIMEOUT -> null (a hang is not a wall)', quotaWallKind({ kind: 'TIMEOUT', message: 'timeout 300000ms' }), null);
  check('quotaWallKind: HTTP_500 -> null', quotaWallKind({ kind: 'HTTP_500', message: '500: upstream boom' }), null);
  check('quotaWallKind: HTTP_404 model-not-found -> null', quotaWallKind({ kind: 'HTTP_404', message: '404: model not found' }), null);
  check('quotaWallKind: plain EMPTY_CONTENT -> null', quotaWallKind({ kind: 'EMPTY_CONTENT', message: 'content empty, no reasoning returned' }), null);
  check('quotaWallKind: MISSING_OLLAMA_API_KEY -> null (fail-closed is not a wall)', quotaWallKind({ kind: 'MISSING_OLLAMA_API_KEY', message: 'Missing OLLAMA_API_KEY' }), null);
  check("quotaWallKind: prose mentioning '4290 items' -> null (no bare-number false positive)", quotaWallKind('processed 4290 items'), null);
  check('quotaWallKind: null -> null', quotaWallKind(null), null);

  // 14b. familyOf — display labels + cloud tags both map; receipted-local -> local; unknown -> itself.
  check("familyOf: 'Gemini 3.5 Flash (Medium)' -> gemini-flash", familyOf('Gemini 3.5 Flash (Medium)'), 'gemini-flash');
  check("familyOf: 'Gemini 3.1 Pro (High)' -> gemini-pro", familyOf('Gemini 3.1 Pro (High)'), 'gemini-pro');
  check("familyOf: 'Claude Sonnet 4.6 (Thinking)' -> claude-agy", familyOf('Claude Sonnet 4.6 (Thinking)'), 'claude-agy');
  check("familyOf: 'Claude Opus 4.6 (Thinking)' -> claude-agy (one agy-Claude pool)", familyOf('Claude Opus 4.6 (Thinking)'), 'claude-agy');
  check("familyOf: 'GPT-OSS 120B (Medium)' -> gpt-oss", familyOf('GPT-OSS 120B (Medium)'), 'gpt-oss');
  check('familyOf: cloud tag kimi-k2.6 -> kimi', familyOf('kimi-k2.6'), 'kimi');
  check('familyOf: cloud tag glm-5.2 -> glm', familyOf('glm-5.2'), 'glm');
  check('familyOf: cloud tag deepseek-v4-pro -> deepseek', familyOf('deepseek-v4-pro'), 'deepseek');
  check('familyOf: cloud tag minimax-m2.7 -> minimax', familyOf('minimax-m2.7'), 'minimax');
  check('familyOf: cloud tag qwen3.5 -> qwen', familyOf('qwen3.5'), 'qwen');
  check('familyOf: nemotron-3-super:latest -> nemotron (tag suffix stripped)', familyOf('nemotron-3-super:latest'), 'nemotron');
  check('familyOf: gpt-oss:120b-cloud -> gpt-oss (cloud suffix stripped)', familyOf('gpt-oss:120b-cloud'), 'gpt-oss');
  check('familyOf: gemini-3-flash-preview slug -> gemini-flash', familyOf('gemini-3-flash-preview'), 'gemini-flash');
  check('familyOf: gemini-3-ultra slug -> gemini-pro (maps the Pro tier, like SEAT_TO_AGY_MODEL)', familyOf('gemini-3-ultra'), 'gemini-pro');
  check('familyOf: gemma4:31b (receipted-local override) -> local', familyOf('gemma4:31b'), 'local');
  check('familyOf: unknown model is its OWN family (never shadows a known pool)', familyOf('laguna-xs-2.1:q8_0'), 'laguna-xs-2.1:q8_0');

  // 14c. wallCooldownMs — kind-proportional, bounded, nothing invented.
  check('wallCooldownMs: 429 -> 30min', wallCooldownMs('429'), 30 * 60 * 1000);
  check('wallCooldownMs: session-wall -> 30min', wallCooldownMs('session-wall'), 30 * 60 * 1000);
  check('wallCooldownMs: weekly-wall -> 6h', wallCooldownMs('weekly-wall'), 6 * 60 * 60 * 1000);
  check('wallCooldownMs: unknown kind -> 0 (no cooldown invented)', wallCooldownMs(null), 0);

  // 14d. Recording upsert + hold consult (temp store via MUEZZIN_QUOTA_WALL_FILE — set at entry).
  {
    const wallFile = process.env.MUEZZIN_QUOTA_WALL_FILE;
    try { unlinkSync(wallFile); } catch { }
    const t0w = Date.now();
    recordQuotaWall('kimi', '429', t0w);
    let db = JSON.parse(readFileSync(wallFile, 'utf8'));
    check("recordQuotaWall: first record creates the family entry (kind='429')", db.kimi?.kind, '429');
    check('recordQuotaWall: first record count=1', db.kimi?.count, 1);
    check('recordQuotaWall: cooldownUntil = lastTs + 30min', Date.parse(db.kimi.cooldownUntil) - t0w, 30 * 60 * 1000);
    recordQuotaWall('kimi', 'weekly-wall', t0w + 1000);
    db = JSON.parse(readFileSync(wallFile, 'utf8'));
    check('recordQuotaWall: upsert increments count', db.kimi?.count, 2);
    check('recordQuotaWall: upsert carries the latest kind (weekly-wall)', db.kimi?.kind, 'weekly-wall');
    check('recordQuotaWall: upsert extends cooldownUntil to 6h', Date.parse(db.kimi.cooldownUntil) - (t0w + 1000), 6 * 60 * 60 * 1000);
    recordQuotaWall('glm', 'session-wall', t0w);
    db = JSON.parse(readFileSync(wallFile, 'utf8'));
    check('recordQuotaWall: families co-exist (kimi untouched by the glm record)', db.kimi?.count === 2 && db.glm?.count === 1, true);
    check('quotaWallHold: live cooldown -> entry returned', quotaWallHold('kimi', t0w + 2000)?.kind, 'weekly-wall');
    check('quotaWallHold: expired cooldown -> null (lane free to run)', quotaWallHold('kimi', t0w + 1000 + 6 * 60 * 60 * 1000 + 1), null);
    check('quotaWallHold: unknown family -> null', quotaWallHold('never-walled', t0w), null);
    check('quotaWallHold: missing store -> null (fails OPEN into normal dispatch)', (() => { try { unlinkSync(wallFile); } catch { } return quotaWallHold('kimi', t0w); })(), null);
    // corruption / unwritable-path classes must NEVER throw (recording must never break dispatch)
    writeFileSync(wallFile, 'NOT JSON {{{{');
    let threw = false;
    try { recordQuotaWall('deepseek', '429', t0w); } catch { threw = true; }
    check('recordQuotaWall: corrupt store never throws (re-seeded — cache, not substrate)', threw, false);
    check('recordQuotaWall: corrupt store re-seeded with the new entry', JSON.parse(readFileSync(wallFile, 'utf8')).deepseek?.count, 1);
    writeFileSync(wallFile, 'NOT JSON {{{{');
    threw = false; let heldCorrupt;
    try { heldCorrupt = quotaWallHold('deepseek', t0w); } catch { threw = true; }
    check('quotaWallHold: corrupt store never throws, reads null (fail-open)', !threw && heldCorrupt === null, true);
    const blocker = join(tmpdir(), 'muezzin-qw-blocker.txt');
    writeFileSync(blocker, 'a FILE, so mkdir of a path under it must fail');
    process.env.MUEZZIN_QUOTA_WALL_FILE = join(blocker, 'sub', 'quota-walls.json');
    threw = false; let recNull;
    try { recNull = recordQuotaWall('minimax', '429', t0w); } catch { threw = true; }
    check('recordQuotaWall: unwritable path never throws (returns null)', !threw && recNull === null, true);
    process.env.MUEZZIN_QUOTA_WALL_FILE = wallFile;
    try { unlinkSync(blocker); } catch { }
    try { unlinkSync(wallFile); } catch { }
  }

  // 14e. LANE-LEVEL CONSULT + RECORD (mocked fetch; zero live calls; keys saved as opaque handles).
  await (async () => {
    const realFetch = globalThis.fetch;
    const savedKey = process.env.OLLAMA_API_KEY;         // opaque save/restore handle — NEVER logged
    const savedKey2 = process.env.OLLAMA_CLOUD_API_KEY;  // opaque save/restore handle — NEVER logged
    ...
    try {
      process.env.OLLAMA_API_KEY = 'sk-selftest-fake-key';
      delete process.env.OLLAMA_CLOUD_API_KEY;
      const mkFetch = (urls) => async (url, opts) => {
        urls.push(String(url));
        const parsed = (() => { try { return JSON.parse(opts?.body || '{}'); } catch { return {}; } })();
        if (String(url).includes('ollama.com'))
          return { ok: true, status: 200, async json() { return { message: { content: 'cloud ran' }, prompt_eval_count: 1, eval_count: 1 }; }, async text() { return ''; } };
        return { ok: true, status: 200, async json() { return { choices: [{ message: { content: `ok model=${parsed.model}` } }], usage: { prompt_tokens: 1, completion_tokens: 1 } }; }, async text() { return 'ok'; } };
      };

      // WALLED cloud family -> lane SKIPPED (hb attempt-skip in the spec format), local safe rail, never throws.
      const untilIso = new Date(Date.now() + 123456789).toISOString();   // unique per run -> precise hb assertion despite the append-only log
      writeFileSync(wallFile, JSON.stringify({ minimax: { kind: 'weekly-wall', count: 3, lastTs: new Date().toISOString(), cooldownUntil: untilIso } }));
      let urls = [];
      globalThis.fetch = mkFetch(urls);
      let rWalled;
      try { rWalled = await dispatchWithWaterfall({ model: 'minimax-m2.7', messages: [{ role: 'user', content: 'x' }] }); }
      catch (e) { rWalled = { provider: `threw:${e.kind}` }; }
      check('walled cloud family: ZERO ollama.com fetches (lane skipped)', urls.filter((u) => u.includes('ollama.com')).length, 0);
      check('walled cloud family: falls through to provider=ollama-local (never throws)', rWalled.provider, 'ollama-local');
      check('walled cloud family: hb carries the exact attempt-skip line',
        readFileSync(process.env.MUEZZIN_HB_FILE, 'utf8')
          .includes(`attempt-skip provider=ollama-cloud model=minimax-m2.7 kind=QUOTA_WALL family=minimax until=${untilIso}`), true);

      // EXPIRED cooldown -> lane RUNS again (the skip is bounded, never a permanent blacklist).
      writeFileSync(wallFile, JSON.stringify({ minimax: { kind: 'weekly-wall', count: 3, lastTs: new Date().toISOString(), cooldownUntil: new Date(Date.now() - 1000).toISOString() } }));
      urls = [];
      globalThis.fetch = mkFetch(urls);
      let rExpired;
      try { rExpired = await dispatchWithWaterfall({ model: 'minimax-m2.7', messages: [{ role: 'user', content: 'x' }] }); }
      catch (e) { rExpired = { provider: `threw:${e.kind}` }; }
      check('expired cooldown: cloud lane RUNS (provider=ollama-cloud)', rExpired.provider, 'ollama-cloud');
      check('expired cooldown: exactly one ollama.com fetch', urls.filter((u) => u.includes('ollama.com')).length, 1);

      // TERMINAL 429 RECORDS: cloud 429 weekly body -> lane throws -> wall recorded -> local safe rail.
      try { unlinkSync(wallFile); } catch { }
      urls = [];
      globalThis.fetch = async (url, opts) => {
        urls.push(String(url));
        if (String(url).includes('ollama.com'))
          return { ok: false, status: 429, async text() { return 'you have reached your weekly usage limit'; } };
        const parsed = (() => { try { return JSON.parse(opts?.body || '{}'); } catch { return {}; } })();
        return { ok: true, status: 200, async json() { return { choices: [{ message: { content: `ok model=${parsed.model}` } }], usage: { prompt_tokens: 1, completion_tokens: 1 } }; }, async text() { return 'ok'; } };
      };
      let rRec;
      try { rRec = await dispatchWithWaterfall({ model: 'minimax-m2.7', messages: [{ role: 'user', content: 'x' }] }); }
      catch (e) { rRec = { provider: `threw:${e.kind}` }; }
      check('terminal cloud 429: dispatch still resolves on the local safe rail', rRec.provider, 'ollama-local');
      const rec = JSON.parse(readFileSync(wallFile, 'utf8'));
      check('terminal cloud 429: weekly wall RECORDED for family minimax', rec.minimax?.kind, 'weekly-wall');
      check('terminal cloud 429: count=1 on first record', rec.minimax?.count, 1);
      // ...and the NEXT dispatch of the SAME FAMILY (different model) now skips the lane.
      urls = [];
      globalThis.fetch = mkFetch(urls);
      let rAfter;
      try { rAfter = await dispatchWithWaterfall({ model: 'minimax-m3', messages: [{ role: 'user', content: 'x' }] }); }
      catch (e) { rAfter = { provider: `threw:${e.kind}` }; }
      check('recorded wall gates the WHOLE family (minimax-m3 skips too, rides local)',
        rAfter.provider === 'ollama-local' && urls.filter((u) => u.includes('ollama.com')).length === 0, true);

      // 14f. AGY-LABEL AUTHORITATIVE GATE (atv-1.S1 fix, 2026-07-08). Receipted bug: a display
      // label whose agy lane failed (PROMPT_TOO_LONG / quota wall) fell into the LOCAL lane —
      // ollama 400 "invalid model name" x4 heals, then "no claude-tier mapping" (mission-events
      // 11:57:06-07, result.json:9). Fixed behavior: a label NEVER dispatches to ollama; its only
      // relief is the Claude tier. These tests run tier=OFF so relief resolution is exercised
      // WITHOUT spawning the live claude exe (no-live-calls constraint); the relief MAPPING
      // itself is unit-checked pure below.
      const savedTier = process.env.MUEZZIN_CLAUDE_TIER;
      process.env.MUEZZIN_CLAUDE_TIER = 'off';
      try {
        // relief mapping (pure — no dispatch): labels map by family; ollama names untouched.
        check('claudeFallbackFor: agy label (validator seat) -> sonnet relief', claudeFallbackFor('GPT-OSS 120B (Medium)', 'validator'), 'sonnet');
        check('claudeFallbackFor: agy Gemini label -> sonnet relief', claudeFallbackFor('Gemini 3.5 Flash (Medium)'), 'sonnet');
        check('claudeFallbackFor: agy Opus-class label -> opus relief', claudeFallbackFor('Claude Opus 4.6 (Thinking)'), 'opus');
        check('claudeFallbackFor: flat ollama map unchanged (deepseek-v4-pro -> sonnet)', claudeFallbackFor('deepseek-v4-pro'), 'sonnet');
        check('claudeFallbackFor: role map unchanged (boundary_auditor glm-5.1 -> haiku)', claudeFallbackFor('glm-5.1', 'boundary_auditor'), 'haiku');

        if (agyAvailable()) {
          // WALLED polarity (runs only where the agy binary exists, since routePrefersAgy gates
          // on agyAvailable()): the skip must NOT fall into the local lane anymore.
          const agyUntil = new Date(Date.now() + 987654321).toISOString();
          writeFileSync(wallFile, JSON.stringify({ 'gemini-flash': { kind: 'session-wall', count: 1, lastTs: new Date().toISOString(), cooldownUntil: agyUntil } }));
          urls = [];
          globalThis.fetch = mkFetch(urls);
          let rAgy;
          try { rAgy = await dispatchWithWaterfall({ model: 'Gemini 3.5 Flash (Medium)', messages: [{ role: 'user', content: 'x' }] }); }
          catch (e) { rAgy = { provider: `threw:${e.kind}`, msg: String(e.message) }; }
          const hbAll = readFileSync(process.env.MUEZZIN_HB_FILE, 'utf8');
          check('walled agy label: hb carries the exact attempt-skip line (provider=agy, resolved label)',
            hbAll.includes(`attempt-skip provider=agy model=Gemini 3.5 Flash (Medium) kind=QUOTA_WALL family=gemini-flash until=${agyUntil}`), true);
          check('walled agy label: NO agy attempt-start after the skip (zero spawns)',
            hbAll.slice(hbAll.indexOf(agyUntil)).includes('attempt-start provider=agy-'), false);
          check('walled agy label: NEVER the local lane — fails closed (was provider=ollama-local before the atv-1.S1 fix)', rAgy.provider, 'threw:ALL_FAILED');
          check('walled agy label: zero fetches to ANY ollama lane', urls.length, 0);
          check('walled agy label: the throw names the design rule', rAgy.msg?.includes('agy display label never dispatches to ollama'), true);

          // PROMPT_TOO_LONG polarity — the exact atv-1.S1 signature, replayed offline (dispatchAgy
          // fails CLOSED pre-spawn on prompts over the argv cap; wall file cleared so the lane runs).
          try { unlinkSync(wallFile); } catch { }
          urls = [];
          globalThis.fetch = mkFetch(urls);
          let rLong;
          try { rLong = await dispatchWithWaterfall({ model: 'GPT-OSS 120B (Medium)', messages: [{ role: 'user', content: 'x'.repeat(46000) }] }); }
          catch (e) { rLong = { provider: `threw:${e.kind}`, msg: String(e.message) }; }
          check('PROMPT_TOO_LONG agy label (atv-1.S1 replay): NEVER the local lane — fails closed', rLong.provider, 'threw:ALL_FAILED');
          check('PROMPT_TOO_LONG agy label: zero ollama fetches (no 400-burn heals)', urls.length, 0);
          check('PROMPT_TOO_LONG agy label: the throw carries the agy cause (argv cap)', rLong.msg?.includes('argv cap'), true);

          // localOnly + label polarity: the label outranks the flag (the OTHER waterfall entrance).
          urls = [];
          globalThis.fetch = mkFetch(urls);
          let rLoc;
          try { rLoc = await dispatchWithWaterfall({ model: 'GPT-OSS 120B (Medium)', messages: [{ role: 'user', content: 'x'.repeat(46000) }] }, { localOnly: true }); }
          catch (e) { rLoc = { provider: `threw:${e.kind}` }; }
          check('localOnly + agy label: label wins over the flag — zero local fetches, fails closed', rLoc.provider === 'threw:ALL_FAILED' && urls.length === 0, true);
        } else {
          console.log('  SKIP  agy-label gate lane tests (agy binary not present on this machine)');
        }

        // NON-LABEL polarity: a plain local tag still rides ollama-local (the gate never leaks
        // onto ollama names — tier off here too, proving the local lane itself is untouched).
        urls = [];
        globalThis.fetch = mkFetch(urls);
        let rTag;
        try { rTag = await dispatchWithWaterfall({ model: 'qwen3.6:27b', messages: [{ role: 'user', content: 'x' }] }); }
        catch (e) { rTag = { provider: `threw:${e.kind}` }; }
        check('non-label local tag: still rides provider=ollama-local (gate does not leak)', rTag.provider, 'ollama-local');
      } finally {
        if (savedTier === undefined) delete process.env.MUEZZIN_CLAUDE_TIER; else process.env.MUEZZIN_CLAUDE_TIER = savedTier;
      }
    } finally {
      restore();
    }
