...
const T = {
  // STATUS_DEAD_MS is MIRRORED in conduct-beat-local.mjs (ghost-lane staleness filter,
  // fork intake 2 — that file never imports this one by design). If this value changes,
  // change the mirror with it.
...
export function sweep(base = HERE, now = Date.now(), routeFile = process.env.AGY_MUEZZIN_ROUTE || path.join(HERE, 'muezzin-route.json'), { sightFn = checkSearxngSight, cgAgeFn = () => checkCgFreshness(now), worktreeReposFn = () => WORKTREE_REPOS, gitFn = null, modelTagsFn = fetchOllamaTags } = {}) {
  ...
  // (ghost-lane staleness review, fork intake 2: already staleness-aware — daemonAlive
  // embeds statusAge < STATUS_DEAD_MS above, so a stale status's lanes never reach here.)
  ...
  // GHOST-LANE STALENESS (fork intake 2) deliberately NOT applied to STUCK-TASK: its
  // remedy is kill + requeue — a FIRING decision (heal() bares the RUNNING lines so the
  // daemon re-fires them). The intake rule is one-directional: stale lanes[] may only be
  // ignored where a ghost would wrongly REFUSE an action; anything that fires work keeps
  // reading the lane claim as-is. (A stale-ts hung-ALIVE daemon — event loop blocked by
  // a long sync call — is also exactly the case where the stuck lane is real and the
  // kill is owed; the heartbeat-quiet condition below already separates work from hang.)
  ...
  // GHOST-LANE STALENESS (fork intake 2) deliberately NOT applied to this suppression:
  // un-suppressing on a stale ts would FIRE destructive git recovery (checkout over the
  // lane's staged work). A live lane inside a hung daemon blocked on a long sync call
  // produces exactly stale-ts-while-lanes-run, and its staged work would be destroyed —
  // the 13:48Z receipt class this suppression exists for. Stale-open errs safe here:
  // worst case the heal waits one beat past the daemon restart (worktree orphans
  // persist; live work does not). autorun.running covers the killed-daemon case anyway.
...
export function heal(base = HERE, now = Date.now(), { exec = (cmd) => execSync(cmd, { stdio: ['ignore', 'ignore', 'pipe'] }), sightFn, worktreeReposFn, gitFn } = {}) {
  ...
  if (restart) {
    ...
    // GHOST-LANE STALENESS (fork intake 2; receipt: the Gemini Flash audition's beat gate
    // refused a correct restart-daemon because daemon-status.json still listed a lane
    // from a KILLED daemon): a lane claim only counts while the status heartbeat itself
    // is alive (ts fresher than STATUS_DEAD_MS). A stale ts is a dead daemon's last
    // words — its lanes[] are ghosts and must not refuse the restart forever. The
    // FRESH-ts + dead-pid case (killed <5m ago, lane still listed) STILL refuses — that
    // is the ambiguous zombie window the note above defends, and it self-resolves on the
    // next sweep once the ts crosses STATUS_DEAD_MS. This relaxes a REFUSAL-gate only:
    // the restart command is a singleton-guarded Start-Process, not a kill.
    const statusFresh = !!status && age(status.ts, now) < T.STATUS_DEAD_MS;
    const lanesRunning = statusFresh && Array.isArray(status.lanes) && status.lanes.length > 0;
...
function selftest() {
  ...
  // fixture 1d: heal() restart guard — ghost-lane staleness polarities (fork intake 2).
  // (1) STALE ts + lane = a killed daemon's ghost lane -> restart PROCEEDS (audition
  // receipt: a dead daemon has no live lanes; a ghost must not refuse restart forever).
  ...
  const healOpts = { sightFn: sightOk.sightFn, worktreeReposFn: () => [], gitFn: stubGit };
  const h2 = heal(tmp, now, { ...healOpts, exec: () => {} });
  ck(h2.performed.some((p) => p.action === 'restart-daemon') && !h2.performed.some((p) => p.action === 'restart-skipped'), 'heal(): STALE status + ghost lane -> restart PROCEEDS (dead daemon has no live lanes)');
  // (2) FRESH ts + lane = the ambiguous zombie window (killed <5m ago, or the pidfile
  // lies) -> restart REFUSED; self-resolves next sweep once ts crosses STATUS_DEAD_MS.
  writeFileSync(path.join(logs, 'daemon-status.json'), JSON.stringify({ pid: 999999999, state: 'running', lanes: ['missions/live.mission.txt'], queued: 0, ts: new Date(now - 60000).toISOString() }));
  const h2b = heal(tmp, now, { ...healOpts, exec: (cmd) => { if (/muezzin-daemon/.test(String(cmd))) throw new Error('RESTART FIRED WHILE A FRESH LANE WAS RUNNING'); } });
  ck(h2b.performed.some((p) => p.action === 'restart-skipped') && !h2b.performed.some((p) => p.action === 'restart-daemon'), 'heal(): FRESH status + lane -> restart REFUSED (a possibly-live mission is never killed)');
  // (3) FRESH ts + NO lanes -> plain idle restart still proceeds (zero behavior change).
  writeFileSync(path.join(logs, 'daemon-status.json'), JSON.stringify({ pid: 999999999, state: 'running', lanes: [], queued: 0, ts: new Date(now - 60000).toISOString() }));
  const h2c = heal(tmp, now, { ...healOpts, exec: () => {} });
  ck(h2c.performed.some((p) => p.action === 'restart-daemon'), 'heal(): FRESH status + empty lanes -> idle restart proceeds');
