...
// never sleeps: this daemon). Transport: ntfy/Discord webhook — URL pasted by the operator
// into the FORK-LOCAL state file below (fork intake 1, 2026-07-08: re-pointed off the
// parent jurisdiction's state dir; this fork's pushes arm independently). Read PER CALL
// so no restart is needed when it appears. The file ships with a non-URL "UNSET" sentinel,
// which the startsWith('https://') check below turns into a silent no-op until armed.
// Fail-silent and fire-and-forget: a push must never break or delay dispatch.
const WEBHOOK_FILE = path.join(HERE, 'missions', '_logs', 'webhook.txt');
...
// atv- added 2026-07-11: live receipt — missions/_logs/GAP-PRIORITY-HOLD (rewritten
// 2026-07-11) records that this fork's OWN design product namespace (atv-*) never
// matched any prefix here, so atv design missions fired straight through the hold all
// week — every prior widening enrolled the parent's muddytires namespaces and nobody
// enrolled the fork's. Gap-owner/engine/smoke stems (sota-smoketest-agy, engine-*, the
// coming stitch-revival/lint-gate gap missions) stay exempt the same way they always
// have: by absence from this list.
const GAP_HOLD_PRODUCT_PREFIXES = ['mt-', 'muddytires-', 'b13-', 'card-', 'cgsports-', 'quirky-', 'qc-concern-', 'qc-fix-', 'm1-', 'atv-'];
...
// CONDUCTOR-BATON gate (fork ruling 2026-07-07, operator fork GO): TWO daemons now share
// one operator (the Claude-side original and this agy-muezzin fork). Both draining queues
// at machine speed is the exact autonomous-queue hazard the gap-hold ruling named — so the
// baton is a FILE (survives reloads, visible to every instance): this fork's daemon fires
// ONLY while missions/_logs/CONDUCTOR-BATON exists AND its FIRST LINE is exactly
// "agy-muezzin". Missing file, unreadable file, or any other first line = the baton is
// held elsewhere; the fire is SKIPPED non-fatally (same shape as gapHoldSkips: the line
// stays pending, zero attempts burned, one event per mission — never a FAILED mark).
// PURE + exported for the selftest; first line is trimmed so a CRLF file grants the same
// baton an LF file does.
...
async function mainLoop() {
  ...
  const fire = async (raw) => {
    // CONDUCTOR-BATON (fork ruling 2026-07-07): before firing ANY lane, this fork's daemon
    // must hold the baton — missions/_logs/CONDUCTOR-BATON present with first line exactly
    // "agy-muezzin". Held elsewhere -> log once and skip NON-FATALLY (line stays pending,
    // zero attempts, no mark — the gapHoldSkips shape). Checked before every other gate so
    // a baton handoff never half-runs a mission (no FAILED marks minted while unseated).
    {
      let batonTxt = null;
      try { batonTxt = readFileSync(path.join(LOGDIR, 'CONDUCTOR-BATON'), 'utf8'); } catch { /* missing/unreadable -> held elsewhere */ }
      if (!batonAllows(batonTxt)) {
        if (!batonHeldLogged.has(raw)) { batonHeldLogged.add(raw); evt(`BATON-HELD-ELSEWHERE: ${raw} — missions/_logs/CONDUCTOR-BATON ${batonTxt === null ? 'missing' : 'first line is not "agy-muezzin"'}; fork daemon skips the fire (non-fatal, fork ruling 2026-07-07)`); }
        return;
      }
    }
...
if (process.argv.includes('--selftest')) {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // PREFLIGHT-RECEIPT CONTENT-AWARENESS (hunt-item #24, 2026-07-04, GAP-CLOSURE-PLAYBOOK
  // UNIT E5): at >=5 failures, mtime alone used to be sufficient -- a hollow touch or a
  // preflight receipt from a DIFFERENT, earlier failure class satisfied the gate just as
  // well as a genuine fresh dry-run for the CURRENT killing class. Never directly unit-tested
  // before this fix (only exercised live). Five fixtures lock the exact contract.
  {
    const NOW5 = Date.parse('2026-07-03T06:00:00Z');
    const ts5 = (h) => `2026-07-03T0${h}-00-00-000Z`;
    const fiveFailsPlan = [0, 1, 2, 3, 4].map((h) => `pf-${ts5(h)}.md`);   // newest retro at hour 4 (04:00)
    const mk5 = (names, failHead) => ({ readdir: () => names, readHead: () => failHead, now: NOW5 });
    const oldMtime5 = Date.parse('2026-07-02T00:00:00Z'), newMtime5 = Date.parse('2026-07-03T04:30:00Z');
    const preflightFreshMtime = Date.parse('2026-07-03T04:55:00Z');   // newer than the newest retro (04:00)

    // (a) hollow preflight (empty content, fresh mtime) -> NO LONGER satisfies the gate
    ck(retroRepeatBlocked('pf', '/r', newMtime5, {
      ...mk5(fiveFailsPlan, '# RETRO pf — FAILED(plan)'), preflightMtimeMs: preflightFreshMtime, readPreflight: () => '',
    }).blocked === true, 'preflight-gate: HOLLOW preflight (fresh mtime, empty content) no longer satisfies the gate (the exact bug this fixes)');

    // (b) stale preflight covering a DIFFERENT class than the one that most recently killed it
    ck(retroRepeatBlocked('pf', '/r', newMtime5, {
      ...mk5(fiveFailsPlan, '# RETRO pf — FAILED(verify)'), preflightMtimeMs: preflightFreshMtime, readPreflight: () => 'COVERS: FAILED(plan) -- dry-ran the old planning-phase issue',
    }).blocked === true, 'preflight-gate: a preflight covering an OLD class (plan) does not satisfy a NEW killing class (verify) -- a stale preflight cannot satisfy a new failure');

    // (c) preflight genuinely covering the CURRENT killing class + fresh mtime + amended text -> PASSES
    const rrbPass = retroRepeatBlocked('pf', '/r', newMtime5, {
      ...mk5(fiveFailsPlan, '# RETRO pf — FAILED(verify)'), preflightMtimeMs: preflightFreshMtime, readPreflight: () => 'COVERS: FAILED(verify) -- ran scripts/verify-popups-e2e.mjs against the preview, PASS',
    });
    ck(rrbPass.blocked === false && rrbPass.preflighted === true, 'preflight-gate: a preflight genuinely covering the CURRENT killing class passes (the legitimate dry-run path)');

    // (d) preflight content matches but mtime is STALE (older than the newest retro) -> still blocked
    ck(retroRepeatBlocked('pf', '/r', newMtime5, {
      ...mk5(fiveFailsPlan, '# RETRO pf — FAILED(verify)'), preflightMtimeMs: oldMtime5, readPreflight: () => 'COVERS: FAILED(verify) -- old dry-run, before the newest failure',
    }).blocked === true, 'preflight-gate: matching content but STALE mtime (older than the newest retro) still blocks -- content-awareness ADDS a check, does not replace the mtime one');

    // (e) killing class unextractable from the retro header (unexpected format) -> falls back
    // to mtime-only rather than permanently blocking on a check it cannot perform
    const rrbFallback = retroRepeatBlocked('pf', '/r', newMtime5, {
      ...mk5(fiveFailsPlan, '# RETRO pf — FAILED (no parenthesized class)'), preflightMtimeMs: preflightFreshMtime, readPreflight: () => 'anything',
    });
    ck(rrbFallback.blocked === false && rrbFallback.preflighted === true, 'preflight-gate: unextractable killing class -> falls back to mtime-only (fail-open on the CONTENT check, never a permanent block on a check the gate cannot perform)');

    // (f) needsPreflight result surfaces the killingClass so the conductor's report can name it
    const rrbNamed = retroRepeatBlocked('pf', '/r', oldMtime5, mk5(fiveFailsPlan, '# RETRO pf — FAILED(plan)'));
    ck(rrbNamed.blocked === true && rrbNamed.needsPreflight === true && rrbNamed.killingClass === 'plan', 'preflight-gate: a BLOCKED result names the killingClass so the report can tell the conductor exactly what to dry-run');
  }
  // ──────────────────────────────────────────────────────────────────────────
  ...
  // WIDENED AGAIN 2026-07-11: the fork's OWN product namespace was never enrolled —
  // missions/_logs/GAP-PRIORITY-HOLD (rewritten 2026-07-11) receipts atv-* design
  // missions firing straight through the hold all week.
  ck(gapHoldSkips('missions/atv-11-design-pass.S1.S2.mission.txt') === true, 'gap-hold: atv-* held (2026-07-11 receipt: fork design product bypassed the hold all week; real on-disk mission)');
  ck(gapHoldSkips('missions/sota-smoketest-agy.mission.txt') === false, 'gap-hold: smoke mission (sota-smoketest-agy) stays exempt — holding the fork smoke surface during gap work would defeat the ruling exactly like holding engine missions would');

  // ---- CONDUCTOR-BATON gate (fork ruling 2026-07-07): both polarities + edge shapes ----
  ck(batonAllows('agy-muezzin\ngranted 2026-07-07 by the operator\'s fork GO') === true, 'baton: first line "agy-muezzin" -> this fork FIRES (grant body on later lines is fine)');
  ck(batonAllows('agy-muezzin\r\ngranted (CRLF file)') === true, 'baton: CRLF first line still grants (trimmed compare)');
  ck(batonAllows('claude-muezzin\nagy-muezzin') === false, 'baton: held by another daemon -> SKIP (a later line naming this fork never grants)');
  ck(batonAllows(null) === false, 'baton: file missing/unreadable (null) -> SKIP (fail closed, never fire unseated)');
  ck(batonAllows('') === false, 'baton: empty file -> SKIP');
  ck(batonAllows('agy-muezzin-2') === false, 'baton: prefix-collision name is NOT this fork (exact match only)');
