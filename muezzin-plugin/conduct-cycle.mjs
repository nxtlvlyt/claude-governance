...
// ROTTEN-PARK SWEEP (QUEUE item 22, eighth law 2026-07-11: "A PARK IS A HANDOFF, NEVER A
// GRAVE" — receipts: stitch-design-mastery FAILED 2026-06-12 + atv-6-stitch-design FAILED
// 2026-07-09 both parked with diagnoses and NO owned fix; the design pipeline ran a MONTH
// without its designer while the Stitch seat sat healthy on nxtbeast). PARKED exits the
// seventh law's diagnosis-debt radar, so an ownerless park is the failure continuing under
// a calmer name. A park is legal ONLY when its comment names an UNPARK OWNER resolvable to
// live tracked work — heuristic: another missions/*.mission.txt path that exists on disk,
// OR a 'QUEUE item N' / 'ITEM N' token that exists in missions/QUEUE.md. "Pending engine
// batch" prose does NOT qualify. Swept: PARKED-status lines + FAILED lines with park-shaped
// notes (/pending engine|engine batch|PARKED/i — VERBATIM parkedRevivalDue's membership, so
// both park watchers see ONE population; this checks a DIFFERENT invariant: revival asks
// "is it due a look?", this asks "does anything own the exit?"). A PARKED-token-only match
// would miss ITEM 22's own census — get-upgrade ("BLOCKED x8-attempts pending engine
// batch"), studio-tools-card, and both stitch-design-mastery graves carry NO literal
// PARKED token. Judged-closed notes (RESOLVED/SUPERSEDED/FIX: none/DUPLICATE-RETIRED —
// parkedRevivalDue's closed family) are exempt — a closed park owes no exit.
// existsFn/queueTextFn injectable so the selftest runs offline.
export function rottenParks(autorun, base, { existsFn, queueTextFn } = {}) {
  const exists = existsFn || ((p) => { try { return existsSync(p); } catch { return false; } });
  const queueText = (queueTextFn ? queueTextFn() : readText(path.join(base, 'missions', 'QUEUE.md'))) || '';
  const items = [];
  for (const p of autorun.parked || []) items.push({ path: p, note: autorun.notes?.[p] || '' });
  for (const f of autorun.failed || []) {
    const note = autorun.notes?.[f] || '';
    if (/pending engine|engine batch|PARKED/i.test(note)) items.push({ path: f, note });   // parkedRevivalDue's exact membership (line ~272)
  }
  const rotten = [];
  for (const it of items) {
    if (/\bRESOLVED\b|\bSUPERSEDED\b|FIX:\s*none\b|\bDUPLICATE-RETIRED\b/i.test(it.note)) continue;   // judged-closed: no exit owed
    const selfStem = stemOf(it.path);
    // owner form 1: another mission file that exists on disk (a fireable queue line);
    // a note quoting the parked mission's OWN path never counts as its own owner.
    const missionRefs = [...it.note.matchAll(/missions\/[\w./-]+?\.mission\.txt/g)].map((m) => m[0]);
    const missionOwner = missionRefs.some((m) => stemOf(m) !== selfStem && exists(path.join(base, m.replace(/\//g, path.sep))));
    // owner form 2: a numbered engine item that actually exists in missions/QUEUE.md
    // ('ITEM 22 —' headers and 'item-22' prose are both live shapes in the real QUEUE.md).
    const itemRefs = [...it.note.matchAll(/\b(?:QUEUE\s+)?ITEM[ -](\d{1,4})\b/gi)].map((m) => m[1]);
    const itemOwner = itemRefs.some((n) => new RegExp(`\\bITEM[ -]?${n}\\b`, 'i').test(queueText));
    if (!missionOwner && !itemOwner) rotten.push({ path: it.path, stem: selfStem, note: it.note });
  }
  return rotten;
}
...
export function sweep(base = HERE, now = Date.now(), routeFile = path.join(process.env.USERPROFILE || 'C:/Users/marka', '.claude', 'state', 'muezzin-route.json'), { sightFn = checkSearxngSight, cgAgeFn = () => checkCgFreshness(now), worktreeReposFn = () => WORKTREE_REPOS, gitFn = null, modelTagsFn = fetchOllamaTags, githubFn = checkGithubCiSignals, sentryFn = checkSentrySignals } = {}) {
  ...
  // ROTTEN-PARK SWEEP (QUEUE item 22, eighth law): a PARKED line whose comment names no
  // UNPARK OWNER resolvable to live tracked work is diagnosis debt again, due next wake,
  // exactly as if the FAILED mark were fresh. Report-only (same advisory-first discipline
  // as the QUEUE.md UNPARKS counter above): the line IS the debt marker the next wake acts
  // on; fixing the park means naming a real owner (queued mission / numbered item), not
  // silencing the sweep.
  try {
    for (const rp of rottenParks(autorun, base)) {
      report.push(`ROTTEN PARK: ${rp.stem} — named owner not found; re-entered diagnosis debt (eighth law)`);
    }
  ...
  // CHAIN-ON-DONE's discipline just below). signals.json missing/empty is a quiet,
  // report-only skip -- the poller is opt-in until the operator's token + repo list
  // exist on this machine (ITEM 19's operator piece is identity-bound).
  {
...
export function heal(base = HERE, now = Date.now(), { exec = (cmd) => execSync(cmd, { stdio: ['ignore', 'ignore', 'pipe'] }), sightFn, worktreeReposFn, gitFn, githubFn, sentryFn } = {}) {
  ...
  // missions/_logs/GAP-REGISTER.jsonl line, mirroring the CHAIN-ON-DONE performer just
  // above. The cursor is re-read and re-written PER ACTION, immediately after that
...
function selftest() {
  ...
  // fixture: ROTTEN-PARK sweep (QUEUE item 22, eighth law — a park is a handoff, never a
  // grave). Unit contract with injected owner resolution, then the sweep wiring emitting
  // the exact report line the debt system keys on.
  {
    const rpAu = parseAutorun(
      'PARKED missions/rp-mission-owner.mission.txt  <!-- 2026-07-11 parked; unpark owner: missions/rp-owner.mission.txt -->\n' +
      'PARKED missions/rp-item-owner.mission.txt  <!-- 2026-07-11 parked; unpark owner: QUEUE item 22 -->\n' +
      'PARKED missions/rp-prose.mission.txt  <!-- 2026-07-11 parked pending engine batch -->\n' +
      'PARKED missions/rp-ghost-owner.mission.txt  <!-- 2026-07-11 parked; unpark owner: missions/rp-gone.mission.txt -->\n' +
      'PARKED missions/rp-closed.mission.txt  <!-- 2026-07-11 parked. RESOLVED 2026-07-11: landed by hand -->\n' +
      'FAILED missions/rp-failed-parked.mission.txt  <!-- diagnosed; PARKED 2026-07-11 pending capability -->\n' +
      'FAILED missions/rp-engine-blocked.mission.txt  <!-- BLOCKED x8-attempts pending engine batch: no batch line queued anywhere -->\n');
    const rpOpts = {
      existsFn: (p) => /rp-owner\.mission\.txt$/.test(p),   // only the named owner file "exists" on disk
      queueTextFn: () => 'ITEM 22 — PARK-OWNER INVARIANT: A PARK WITHOUT AN OWNED EXIT IS A GRAVE.\n',
    };
    const rp = rottenParks(rpAu, tmp, rpOpts);
    ck(!rp.some((x) => x.stem === 'rp-mission-owner'), 'rotten-park: owner naming a missions/*.mission.txt that EXISTS on disk passes (handoff, not grave)');
    ck(!rp.some((x) => x.stem === 'rp-item-owner'), 'rotten-park: owner naming a QUEUE item that exists in missions/QUEUE.md passes');
    ck(rp.some((x) => x.stem === 'rp-prose'), 'rotten-park: prose owner ("pending engine batch") flags ROTTEN — untracked prose is the failure continuing under a calmer name');
    ck(rp.some((x) => x.stem === 'rp-ghost-owner'), 'rotten-park: an owner mission path that does NOT exist on disk flags ROTTEN (named is not resolved)');
    ck(!rp.some((x) => x.stem === 'rp-closed'), 'rotten-park: judged-closed (RESOLVED) park is exempt — a closed park owes no exit');
    ck(rp.some((x) => x.stem === 'rp-failed-parked'), 'rotten-park: a FAILED line whose comment says PARKED is swept too — the status token is not the loophole');
    ck(rp.some((x) => x.stem === 'rp-engine-blocked'), 'rotten-park: FAILED "pending engine batch" with NO literal PARKED token is swept — the get-upgrade/stitch-grave class named in the ITEM 22 census');
    // sweep wiring: no QUEUE.md and no owner file exist in the fixture base, so the prose
    // park must surface as the EXACT eighth-law report line.
    writeFileSync(path.join(tmp, 'missions', 'AUTORUN.md'), '# q\nPARKED missions/rp-prose.mission.txt  <!-- 2026-07-11 parked pending engine batch -->\n');
    const rRp = sweep(tmp, now, noRoute, sightOk);
    ck(rRp.report.includes('ROTTEN PARK: rp-prose — named owner not found; re-entered diagnosis debt (eighth law)'), 'rotten-park: sweep emits the exact ROTTEN PARK line (stem + eighth-law debt re-entry)');
  }
  ...
  // affected item to ok:false without crashing the check or the sweep.
  {
    ...
    ck(h1f2.performed.filter((p) => p.action === 'external-signal-filed').length === 2, 'heal(): both new signals filed (CI failure + Sentry issue)');
    const inboxAfter = readText(path.join(tmp, 'missions', 'INBOX.md'));
    ck(inboxAfter.length > inboxBefore.length && /EXTERNAL SIGNAL: NEW CI FAILURE/.test(inboxAfter) && /EXTERNAL SIGNAL: NEW SENTRY ISSUE/.test(inboxAfter), 'heal(): dated INBOX.md entries appended for both signal classes');
    const registerText = readText(path.join(logs, 'GAP-REGISTER.jsonl'));
    ck(/gap-ci-failure-/.test(registerText) && /gap-sentry-issue-/.test(registerText), 'heal(): GAP-REGISTER.jsonl lines appended for both signal classes');
    ...
    ck(ghThrown.ok === true && ghThrown.repos['nxtlvlyt/muddytires-pages'].ok === false, 'checkGithubCiSignals: a throwing probe degrades that repo to ok:false without crashing');
    const snThrown = checkSentrySignals({ probe: throwProbe, projects: [{ org: 'abass-inc', slug: 'muddytires', id: '4511612938551296' }] });
    ck(snThrown.ok === true && snThrown.projects['muddytires'].ok === false, 'checkSentrySignals: a throwing probe degrades that project to ok:false without crashing');
