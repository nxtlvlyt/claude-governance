...
import { dispatchSeat, recognizeClaudeModel, isCloudModelTag } from './seat_dispatch.mjs';
import { isAgyModelLabel } from './agy_dispatch.mjs';
...
// EXECUTOR TIER-0 FLOOR (atv-1.S1 fix, 2026-07-08 — the single source of truth for "which seat
// does an unforced executor step start on?", used by implementStep AND orchestrate's escalation
// dedup, same de-dup rationale as isProseTarget above).
// PROSE-FAITHFUL FLOOR (2026-06-17; guardian+laguna APPROVE): content-FABRICATION (guides
// invented topics, about invented stats) traced to LOCAL CODE models (qwen/kimi) authoring
// PROSE. seat-record: sonnet emission = 0 fab; qwen 5, kimi 2. Route PROSE targets
// (.html/.md/.txt) to the faithful seat (sonnet); CODE/config stays on the mode's coder.
// MODE SEAT OUTRANKS THE FLOOR FOR FRONTIER SEATS (atv-1.S1 receipts): the floor exists to keep
// the fabrication-prone LOCAL-tag class off prose — but it was hardcoding 'sonnet' over EVERY
// mode, silently bypassing the seat table (atv-1.S1: all 7 research steps rode sonnet while
// agy-hybrid seated glm-5.2, chosen by the 2026-07-08 SOTA verdict partly FOR big-page
// full-file emission). A FRONTIER-class mode executor (Claude name, agy display label, or
// censused cloud tag) is not the hazard class the floor guards against, so the MODE seat wins.
// Local-tag executors (balance / local-heavy / no-mode) keep the sonnet floor byte-for-byte.
export function executorFloorSeat(target) {
  const modeSeat = pickSeat('executor', 'north-mini-code-toolcall');   // default floor renamed kimi-k2.7-code -> north-mini-code-toolcall 2026-07-03 (honest name, same blob 429d372cb9f6)
  const frontierSeat = Boolean(recognizeClaudeModel(modeSeat) || isAgyModelLabel(modeSeat) || isCloudModelTag(modeSeat));
  return (isProseTarget(target) && !frontierSeat && process.env.MUEZZIN_CLAUDE_TIER === 'on') ? 'sonnet' : modeSeat;
}
...
export async function implementStep(step, cwd, { dispatch = dispatchSeat, model = null, today = '2026-06-09', maxContinuations = Number(process.env.MUEZZIN_MAX_CONTINUATIONS) || 6, codeRepo = false, repoRoot = null, allowFiles = [] } = {}) {
  ...
  if (!model) {
    ...
    // PROSE-FAITHFUL FLOOR + MODE-SEAT PRECEDENCE: extracted to executorFloorSeat (top of file)
    // 2026-07-08 (atv-1.S1 fix) so implementStep and orchestrate's escalation dedup share ONE
    // tier-0 seat decision — the full fabrication-receipt rationale lives on the function.
    const floor = executorFloorSeat(step.target_files?.[0]);
...
if (process.argv[1]?.endsWith('executor.mjs')) {
  ...
  // model arg still wins (every test above passes one, so all are unaffected).
  {
    ...
    // PROSE targets route to the faithful seat (sonnet) whenever the mode's executor is a
    // LOCAL-tag coder (2026-06-17 prose-split, fabrication fix — the hazard class):
    ck(await captureSeatModel('local-heavy', 'page.md') === 'sonnet', 'PROSE-FAITHFUL FLOOR: a .md target -> sonnet under local-heavy (local-tag executor = the fabrication hazard class)');
    ck(await captureSeatModel('__none__', 'index.html') === 'sonnet', 'PROSE-FAITHFUL FLOOR: a .html target -> sonnet with no mode (default local-tag floor)');
    // MODE SEAT OUTRANKS THE FLOOR FOR FRONTIER SEATS (atv-1.S1 fix, 2026-07-08): agy-hybrid
    // seats glm-5.2 (censused cloud tag, SOTA-verdict executor) — the sonnet hardcode used to
    // bypass it on every prose step (atv-1.S1: 7/7 research .md steps rode sonnet).
    ck(await captureSeatModel('agy-hybrid') === 'glm-5.2', 'SEATING MODE agy-hybrid: executor floor is the mode seat glm-5.2 (code target)');
    ck(await captureSeatModel('agy-hybrid', 'competitor-report.md') === 'glm-5.2', 'MODE SEAT OUTRANKS PROSE FLOOR: agy-hybrid .md (research-class emission) -> glm-5.2, NOT sonnet (atv-1.S1 fix)');
    ck(await captureSeatModel('anthropic-heavy', 'page.md') === 'sonnet', 'FRONTIER mode seat that IS sonnet: anthropic-heavy prose -> sonnet (unchanged, via the mode seat not the hardcode)');
  }
