...
// ---- DECLARED-DEP VERBATIM SURVIVAL (dep-loss receipted 2026-07-08, atv-1 S1.S1) --------
// The parent mission DECLARED absolute context_dependencies verbatim in its step lines
// ("context_dependencies: C:/Users/marka/agy-muezzin/data/atv-corpus/firesticktricks.md"),
// but the plan is MODEL-AUTHORED — nothing mechanically forced the planner to copy the
// declared paths into the plan's context_dependencies, and after the hajj autosplit chain
// (parent -> S1 -> S1.S1) the grandchild's executor INVENTED reference/firesticktricks.com.txt
// and profiled against nothing. Advice-without-enforcement is this system's documented
// failure mode, so this is the MECHANICAL half: parse the dep declarations out of the
...
// lives in mission_split.mjs emitSubMissions, which now serializes each child step's
// context_dependencies verbatim into the child mission text — so the guarantee survives
// autosplit RECURSION (child text -> child plan -> grandchild text -> ...).
//
// Declaration convention (the parent's own shape, and what emitSubMissions now emits):
...
// ---- DECLARED-VALIDATION VERBATIM SURVIVAL (e1cba9e residual, closed 2026-07-08) --------
// SAME LOSS SHAPE as context_dependencies (commit e1cba9e): the parent mission DECLARED
// per-step validation_command gates verbatim (atv-1's anti-fabrication FETCH-RECEIPT
// Select-String gates), but the plan is MODEL-AUTHORED and an autosplit child is RE-PLANNED
// from its own text — so a gate absent from the child text is gone, and the child's planner
// RE-INVENTS a weaker witness (a bare Test-Path where the mission demanded the FETCH-RECEIPT
// content gate). This is the MECHANICAL half: parse the declared gates out of the mission
// text and re-impose any the planner re-authored, BEFORE validation, on every plan path
// (single architect + panel integrator, via runQueueLoop). The other half lives in
// mission_split.mjs emitSubMissions, which serializes each child step's validation_command
// verbatim as a following indented line — the parent's own convention — so the guarantee
// survives autosplit RECURSION (parent -> S1 -> S1.S1 -> ...).
//
// Declaration convention (the parent's own shape — atv-1 — and what emitSubMissions emits):
//   ... [edit|command|verify] <target[, target]> [; context_dependencies: <dep[, dep]>]
//       validation_command: <the complete witness command, verbatim to end of line>
// The command line binds to the NEAREST PRECEDING [action] step-target line; a blank line
// or a new numbered step line breaks the binding, and a binding is consumed by the first
// command line it receives — so prose paragraphs can never inherit a stale step. A same-line
// "... [action] <target> ; validation_command: <cmd>" also counts. Deps used a token filter
// (looksLikeDepPath) to reject prose; a command is arbitrary text, so here the anti-prose
// filter is STRUCTURAL: an unbound "validation_command:" line parses as NOTHING.
const ACTION_TARGETS_RE = /\[(edit|command|verify)\]\s*([^;\r\n]*)/i;

// parseDeclaredValidations(mission) -> [{ command: verbatim, targets: [verbatim...], action }]
export function parseDeclaredValidations(mission) {
  const decls = [];
  let bound = null;   // nearest preceding step-target line still eligible to receive a command
  for (const line of String(mission || '').split(/\r?\n/)) {
    if (!line.trim()) { bound = null; continue; }                        // blank line breaks binding
    const am = line.match(ACTION_TARGETS_RE);
    if (am) {
      const action = am[1].toLowerCase();
      const targets = am[2].split(',').map((s) => s.trim()).filter(Boolean);
      const sm = line.match(/\bvalidation_command\s*:\s*(.+)$/i);        // same-line form
      if (sm && targets.length && sm[1].trim()) { decls.push({ command: sm[1].trim(), targets, action }); bound = null; }
      else bound = targets.length ? { targets, action } : null;
      continue;
    }
    if (/^\s*\d+\.\s/.test(line)) { bound = null; continue; }            // new numbered step (its [action] line not seen yet)
    const vm = line.match(/^[ \t]*validation_command\s*:\s*(.+)$/i);     // next-line form (the atv-1 shape)
    if (vm && bound && vm[1].trim()) { decls.push({ command: vm[1].trim(), targets: bound.targets, action: bound.action }); bound = null; }
  }
  return decls;
}

// injectDeclaredValidations(queue, mission, { research, codeRepo }) — mutates queue in
// place; returns { injected, decls }. Pairing: a declaration lands on the plan step(s)
// matching BOTH its action_type and a declared target; if none, a target-only match still
// receives it (the planner changed the action but kept the deliverable). NO edit-step
// fallback — deliberately ASYMMETRIC with injectDeclaredDeps: deps are ADDITIVE read-only
// inputs (over-providing is bounded), but a validation_command is a scalar OVERWRITE, and
// spraying one gate over every step would destroy correct witnesses. A fully-renamed target
// keeps the planner's command (mission lint + the verdict panel still police it).
// Floor containment MIRRORED, not bypassed (the dep fix's escapesSandbox principle): a
// declared command that would trip a validateMicroAction floor the planner's own command
// does not is skipped, never injected — the backstop runs on every repair attempt and must
// not deadlock the repair loop on a mission-authoring error.
export function injectDeclaredValidations(queue, mission, { research = false, codeRepo = false } = {}) {
  const decls = parseDeclaredValidations(mission);
  const steps = Array.isArray(queue?.steps) ? queue.steps : [];
  let injected = 0;
  if (!decls.length || !steps.length) return { injected, decls: decls.length };
  const norm = (p) => String(p).toLowerCase().replace(/\\/g, '/');
  const vopts = { research, codeRepo };
  for (const { command, targets, action } of decls) {
    if (!command || !targets.length) continue;
    const tset = new Set(targets.map(norm));
    const onTarget = (s) => (s?.target_files || []).some((t) => tset.has(norm(t)));
    let recipients = steps.filter((s) => s?.action_type === action && onTarget(s));
    if (!recipients.length) recipients = steps.filter(onTarget);
    for (const s of recipients) {
      if (String(s.validation_command || '') === command) continue;      // idempotent — already verbatim
      const before = validateMicroAction(s, 0, vopts);
      const after = validateMicroAction({ ...s, validation_command: command }, 0, vopts);
      if (after.some((e) => !before.includes(e))) continue;              // declared gate trips a floor the planner's didn't -> skip (no deadlock)
      s.validation_command = command;
      injected++;
    }
  }
  return { injected, decls: decls.length };
...
export function validateMicroQueue(queue, opts = {}) {
  ...
  if (opts.codeRepo && Array.isArray(opts.allowFiles) && opts.allowFiles.length) {
    ...
    (queue.steps || []).forEach((s) => {
      ...
      // (Claude-side e5ddc1c cherry-pick): `errors => consoleMessages.push(e)` matched as
      // creating 'consoleMessages.push', `m => m.toLowerCase()` as 'm.toLowe' (\w{1,6} cap).
...
FINAL-COMMIT IDEMPOTENCY RULE (d1-migrations receipt 2026-07-02: mission looped forever on step-8 "nothing to commit" AFTER its work landed): a final integrate/receipt commit step MUST no-op cleanly when the integration commits already exist — shape: if (git log --oneline -30 | Select-String '<origin-sha-or-feature-name>' -Quiet) { Write-Output ALREADY_COMMITTED } else { git add <allow-files>; git commit -m '<msg>' } — a bare git commit that exits 1 on an empty tree turns a SUCCEEDED mission into a perpetual FAILED.
STANDARDS-GROUNDING RULE (operator ruling 2026-07-02, QUALITY-STANDARD user-facing bar): you are search-grounded — for a USER-FACING deliverable (UI, popup, page, interactive element, PWA/offline behavior), your plan narrative names the external standard/conventions it was planned against (WCAG for interactive/a11y, established map-UX conventions for map features, PWA specs for install/offline) with the source you consulted. The verdict panel reads the deliverable against your cited standard. Best effort includes knowing what the field's bar IS — a user-facing plan with no standards citation is an incomplete plan.
...
async function runQueueLoop(seat, baseFraming, { research = false, codeRepo = false, allowFiles = [], maxRepairs = 2, diagDir = null, diagTag = 'plan', dispatchFn = dispatchSeat, escalationState = { tier: 0, reason: 'initial' }, missionText = null } = {}) {
  ...
  for (let attempt = 0; attempt <= maxRepairs; attempt++) {
    ...
    // DEP-LOSS MECHANICAL BACKSTOP (2026-07-08, atv-1 S1.S1 receipt): BEFORE validation,
    // re-inject any mission-text-declared context_dependencies the model dropped or
    // re-authored (the planner invented reference/*.txt where the mission declared
    // data/atv-corpus/*.md absolutely). Runs on every attempt so the repair loop can
    // never un-inject them; validation then judges the FINAL queue.
    // VALIDATION-LOSS BACKSTOP (e1cba9e residual, same beat): re-impose any mission-text-
    // declared per-step validation_command the model re-authored (the child re-plan used to
    // re-invent atv-1's FETCH-RECEIPT gates as bare Test-Path) — declared gates win verbatim.
    if (queue && missionText) {
      injectDeclaredDeps(queue, missionText, { research });
      injectDeclaredValidations(queue, missionText, { research, codeRepo });
    }
...
export const PANEL_ARCHITECTS = ['north-mini-code-toolcall:latest', 'qwen3.6:35b', 'granite4.1:30b'];  // NO-CLOUD ruling 2026-07-02: local nxtbeast trio, 3 labs (Cohere-North/Alibaba/IBM). HONEST-NAME RENAME 2026-07-03 (operator: recurring cross-instance kimi arguments traced to the lying tag): seat A was 'kimi-k2.7-code:latest', an ALIAS of this same blob (digest 429d372cb9f6, /api/show general.name=North, cohere2moe 30.5B — never Moonshot; real Kimi K2.7 is 1.04T, cloud-only). Same digest = zero behavior change; the old tag stays resolvable on the server until retired on a clear lane.
...
export async function deconstructPanel(mission, {
  ...
  const archFraming =
    ...
    `Produce a PLAN NARRATIVE: the ordered steps you would decompose this mission into, each as a short line stating WHAT the step does, its action_type (edit ONE impl file | command | verify), its single target file (cwd-relative), and how it is witnessed (a validation_command). COMMAND/VERIFY STEPS ARE ENGINE-EXECUTED: for action_type 'command' or 'verify' the engine runs ONLY your validation_command line (pwsh, no model dispatched) — validation_command IS the step. Write the COMPLETE working command there (the real fetch/dispatch/relay invocation AND its outcome check, as one expression) — never a bare existence-check standing in for work that happens nowhere else. A check-only validation_command on a command/verify step cannot be safely reconstructed later (receipt: gap-attempt2-validation-replay — a Stitch relay-dispatch step was repeatedly planned with validation_command = only \`Test-Path ...STITCH-RECEIPT.md\`, so each ~4min re-plan's real dispatch never ran and the step died in <1s). Do NOT emit the final json queue — a separate integrator will synthesize the three narratives into the validated queue.\n` +
...
Context: Node + TypeScript project; DB schema in prisma/schema.prisma; tests run with \`node --test\`.`;
  ...
  // scenic.S2 attempt-7 receipt 2026-07-11 (Claude-side e5ddc1c cherry-pick): arrows aren't redirects
...
Maqsad: implement seven modules.`;
  ...
  // -------------------------------- (12) DECLARED-DEP VERBATIM SURVIVAL (dep-loss,
  // receipted 2026-07-08: after the hajj autosplit chain, atv-1 S1.S1 carried ZERO
  // occurrences of the parent's declared data/atv-corpus deps and its executor invented
  // reference/firesticktricks.com.txt). parseDeclaredDeps + injectDeclaredDeps + the
  // runQueueLoop wiring on BOTH plan paths.
  {
    const ABS = 'C:/Users/marka/agy-muezzin/data/atv-corpus/firesticktricks.md';
    const atvMission = [
      'MISSION-CLASS: research',
      'MISSION-ID: M-ATV.T',
      'Maqsad: profile competitors from committed evidence.',
      'Steps:',
      `  1. Profile firesticktricks.com from its committed evidence file. [edit] parts/competitor-firesticktricks.com.md ; context_dependencies: ${ABS}`,
      '  2. Search for more competitors. [edit] parts/competitors-found.md ; context_dependencies: none',
      '',
      'Done means: both parts files exist.',
    ].join('\n');

    // (a) parse: the absolute dep extracted VERBATIM, paired to its same-line target; "none" and
    // prose mentions ("read it via context_dependencies, never write") yield nothing.
    const decls = parseDeclaredDeps(atvMission);
    ck(decls.length === 1 && decls[0].deps.length === 1 && decls[0].deps[0] === ABS, 'DEP-SURVIVAL parse: absolute dep extracted VERBATIM; "none" declaration yields nothing');
    ck(decls[0].targets.length === 1 && decls[0].targets[0] === 'parts/competitor-firesticktricks.com.md', 'DEP-SURVIVAL parse: dep paired to its same-line [edit] target');
    ck(parseDeclaredDeps('read it via context_dependencies, never write to it.\n"context_dependencies":["path"]').length === 0, 'DEP-SURVIVAL parse: prose mention (no colon) + JSON schema fragment parse as NOTHING (no false deps)');

    // (b) inject: a model-authored queue that DROPPED the dep gets it back on the matching step only.
    const droppedQ = { mission_id: 'M-ATV.T', steps: [
      { step_index: 1, description: 'profile firesticktricks', action_type: 'edit', target_files: ['parts/competitor-firesticktricks.com.md'], context_dependencies: [], validation_command: 'Test-Path parts/competitor-firesticktricks.com.md' },
      { step_index: 2, description: 'found competitors', action_type: 'edit', target_files: ['parts/competitors-found.md'], context_dependencies: [], validation_command: 'Test-Path parts/competitors-found.md' },
    ] };
    const inj = injectDeclaredDeps(droppedQ, atvMission, { research: true });
    ck(inj.injected === 1 && droppedQ.steps[0].context_dependencies.length === 1 && droppedQ.steps[0].context_dependencies[0] === ABS, 'DEP-SURVIVAL inject: dropped dep re-injected VERBATIM onto the step targeting its declared file');
    ck(droppedQ.steps[1].context_dependencies.length === 0, 'DEP-SURVIVAL inject: unrelated step NOT polluted when target-pairing matches');
    ck(validateMicroQueue(droppedQ, { research: true }).ok, 'DEP-SURVIVAL inject: the injected queue still passes validateMicroQueue (research absolute-dep rule)');
    const inj2 = injectDeclaredDeps(droppedQ, atvMission, { research: true });
    ck(inj2.injected === 0 && droppedQ.steps[0].context_dependencies.length === 1, 'DEP-SURVIVAL inject: idempotent — a dep already present (verbatim) is never duplicated');

    // (c) fallback: the planner RENAMED the target -> the dep still lands (edit-step fallback;
    // silent loss is impossible), including when the model substituted an INVENTED dep.
    const renamedQ = { mission_id: 'M-ATV.T', steps: [
      { step_index: 1, description: 'profile firesticktricks', action_type: 'edit', target_files: ['parts/firesticktricks-profile.md'], context_dependencies: ['reference/firesticktricks.com.txt'], validation_command: 'Test-Path parts/firesticktricks-profile.md' },
    ] };
    injectDeclaredDeps(renamedQ, atvMission, { research: true });
    ck(renamedQ.steps[0].context_dependencies.includes(ABS), 'DEP-SURVIVAL inject: target renamed + dep re-authored (the invented reference/*.txt class) -> the declared dep STILL lands via the edit-step fallback');

    // (d) class containment MIRRORED: the same absolute decl in a NON-research mission is NOT
    // injected (validateMicroAction would reject it; the backstop must not deadlock the repair loop).
    const crQ = { mission_id: 'M-ATV.T', steps: [
      { step_index: 1, description: 'profile', action_type: 'edit', target_files: ['parts/competitor-firesticktricks.com.md'], context_dependencies: [], validation_command: 'Test-Path parts/competitor-firesticktricks.com.md' },
    ] };
    const injCr = injectDeclaredDeps(crQ, atvMission.replace('MISSION-CLASS: research', 'MISSION-CLASS: code'), { research: false });
    ck(injCr.injected === 0 && crQ.steps[0].context_dependencies.length === 0, 'DEP-SURVIVAL inject: an absolute dep is NOT injected outside research class (mirrors validateMicroAction containment — no repair-loop deadlock)');

    // (e) WIRED through deconstruct() (single-architect path): the fake architect emits a queue
    // whose dep was re-authored to the invented path; the RETURNED plan carries the declared dep.
    {
      const noDepJson = '```json\n' + JSON.stringify({ mission_id: 'M-ATV.T', steps: [
        { step_index: 1, description: 'profile firesticktricks', action_type: 'edit', target_files: ['parts/competitor-firesticktricks.com.md'], context_dependencies: ['reference/firesticktricks.com.txt'], validation_command: 'Test-Path parts/competitor-firesticktricks.com.md' },
      ] }) + '\n```';
      const r = await deconstruct(atvMission, { dispatchFn: async () => ({ content: noDepJson }), maxRepairs: 0 });
      ck(r.ok === true && r.queue.steps[0].context_dependencies.includes(ABS), 'DEP-SURVIVAL wired (deconstruct): model re-authored the dep -> declared dep mechanically injected before validation (not prompt advice)');
    }

    // (f) WIRED through the panel integrator: same guarantee on the panel path.
    {
      const noDepJson = '```json\n' + JSON.stringify({ mission_id: 'M-ATV.T', steps: [
        { step_index: 1, description: 'profile firesticktricks', action_type: 'edit', target_files: ['parts/competitor-firesticktricks.com.md'], context_dependencies: [], validation_command: 'Test-Path parts/competitor-firesticktricks.com.md' },
      ] }) + '\n```';
      const dispatch = async (seat) => seat.role === 'integrator' ? { content: noDepJson, provider: 'test' } : groundedArch;
      const r = await deconstructPanel(atvMission, { dispatchFn: dispatch, maxRepairs: 0, route: 'panel' });
      ck(r.ok === true && r._panel === true && r.queue.steps[0].context_dependencies.includes(ABS), 'DEP-SURVIVAL wired (panel integrator): declared dep injected on the panel path too');
    }
  }

  // -------------------------------- (13) DECLARED-VALIDATION VERBATIM SURVIVAL (the
  // e1cba9e residual, closed 2026-07-08: the SAME serializer/planner loss shape as deps —
  // atv-1 declared per-step anti-fabrication FETCH-RECEIPT Select-String gates, the child
  // re-plan re-invented them as weaker witnesses). parseDeclaredValidations +
  // injectDeclaredValidations + the runQueueLoop wiring on BOTH plan paths.
  {
    const TGT = 'parts/competitor-firesticktricks.com.md';
    const GATE = `if (-not ((Select-String -Path ${TGT} -Pattern 'FETCH-RECEIPT:' -Quiet) -and ((Select-String -Path ${TGT} -Pattern 'FETCHED' -Quiet) -or (Select-String -Path ${TGT} -Pattern 'UNREACHABLE' -Quiet)))) { exit 1 }`;
    const GATE2 = "Select-String -Path parts/competitors-found.md -Pattern 'https?://' -Quiet";
    const vMission = [
      'MISSION-CLASS: research',
      'MISSION-ID: M-ATV.V',
      'Maqsad: profile competitors from committed evidence.',
      'Steps:',
      '  1. Profile firesticktricks.com from its committed evidence file.',
      `     [edit] ${TGT} ; context_dependencies: C:/Users/marka/agy-muezzin/data/atv-corpus/firesticktricks.md`,
      `     validation_command: ${GATE}`,
      `  2. Search for more competitors. [edit] parts/competitors-found.md ; validation_command: ${GATE2}`,
      '',
      'Done means: both parts files exist.',
    ].join('\n');

    // (a) parse: next-line gate extracted VERBATIM (quotes, parens, braces, semicolons intact),
    // paired to its step's target + action; the same-line form also counts.
    const vdecls = parseDeclaredValidations(vMission);
    ck(vdecls.length === 2 && vdecls[0].command === GATE, 'VAL-SURVIVAL parse: the FETCH-RECEIPT gate extracted VERBATIM from the next-line convention (the atv-1 shape)');
    ck(vdecls[0].targets.length === 1 && vdecls[0].targets[0] === TGT && vdecls[0].action === 'edit', 'VAL-SURVIVAL parse: gate paired to its preceding [edit] target line');
    ck(vdecls[1].command === GATE2 && vdecls[1].targets[0] === 'parts/competitors-found.md', 'VAL-SURVIVAL parse: same-line "... [edit] target ; validation_command: cmd" form also parses');
    // anti-prose: an UNBOUND validation_command line (no preceding [action] line), a mid-prose
    // mention, and a JSON schema fragment all parse as NOTHING — pairing IS the filter.
    ck(parseDeclaredValidations('witnessed by their\nvalidation_command: is how steps prove deeds\n"validation_command":"x"').length === 0, 'VAL-SURVIVAL parse: unbound line + prose + JSON fragment parse as NOTHING (no false gates)');
    ck(parseDeclaredValidations(`  1. Author it. [edit] a.md\n\nvalidation_command: Test-Path a.md`).length === 0, 'VAL-SURVIVAL parse: a blank line breaks the binding (a later stray line cannot inherit a stale step)');

    // (b) inject: a planner that RE-INVENTED the gate as a weaker witness (the receipted
    // class) gets the declared gate back VERBATIM on the matching step only; idempotent.
    const weakQ = () => ({ mission_id: 'M-ATV.V', steps: [
      { step_index: 1, description: 'profile firesticktricks', action_type: 'edit', target_files: [TGT], context_dependencies: [], validation_command: `Test-Path ${TGT}` },
      { step_index: 2, description: 'found competitors', action_type: 'edit', target_files: ['parts/competitors-found.md'], context_dependencies: [], validation_command: GATE2 },
    ] });
    const wq = weakQ();
    const vinj = injectDeclaredValidations(wq, vMission, { research: true });
    ck(vinj.injected === 1 && wq.steps[0].validation_command === GATE, 'VAL-SURVIVAL inject: re-invented weak witness OVERWRITTEN with the declared gate VERBATIM (declared wins — the plan is derived, the mission is law)');
    ck(wq.steps[1].validation_command === GATE2, 'VAL-SURVIVAL inject: a step already carrying its declared gate is untouched');
    ck(validateMicroQueue(wq, { research: true }).ok, 'VAL-SURVIVAL inject: the injected queue still passes validateMicroQueue');
    const vinj2 = injectDeclaredValidations(wq, vMission, { research: true });
    ck(vinj2.injected === 0 && wq.steps[0].validation_command === GATE, 'VAL-SURVIVAL inject: idempotent — a gate already present verbatim is never re-counted');

    // (c) action changed but target kept -> the gate still lands (target-only fallback).
    const cmdQ = { mission_id: 'M-ATV.V', steps: [
      { step_index: 1, description: 'produce the profile', action_type: 'command', target_files: [TGT], context_dependencies: [], validation_command: `Test-Path ${TGT}` },
    ] };
    injectDeclaredValidations(cmdQ, vMission, { research: true });
    ck(cmdQ.steps[0].validation_command === GATE, 'VAL-SURVIVAL inject: planner changed the action_type but kept the target -> declared gate still lands (target-only fallback)');

    // (d) fully-RENAMED target -> NOT overwritten (asymmetric with deps BY DESIGN: a scalar
    // overwrite sprayed onto every edit step would destroy correct witnesses).
    const renQ = { mission_id: 'M-ATV.V', steps: [
      { step_index: 1, description: 'profile firesticktricks', action_type: 'edit', target_files: ['parts/firesticktricks-profile.md'], context_dependencies: [], validation_command: 'Test-Path parts/firesticktricks-profile.md' },
    ] };
    injectDeclaredValidations(renQ, vMission, { research: true });
    ck(renQ.steps[0].validation_command === 'Test-Path parts/firesticktricks-profile.md', 'VAL-SURVIVAL inject: fully-renamed target keeps the planner command (NO spray fallback — deliberate asymmetry with deps)');

    // (e) floor containment MIRRORED: a declared gate that would trip a validateMicroAction
    // floor the planner's command does not (pwsh peel-syntax here) is SKIPPED, never injected
    // — the backstop must not deadlock the repair loop on a mission-authoring error.
    const peelMission = '  1. Verify the source commit. [verify] docs/x.md\n     validation_command: git cat-file -e cbb07a5^{commit}';
    const peelQ = { mission_id: 'M-PEEL', steps: [
      { step_index: 1, description: 'verify the source commit', action_type: 'verify', target_files: ['docs/x.md'], context_dependencies: [], validation_command: 'git cat-file -e cbb07a5' },
    ] };
    const peelInj = injectDeclaredValidations(peelQ, peelMission, {});
    ck(peelInj.injected === 0 && peelQ.steps[0].validation_command === 'git cat-file -e cbb07a5', 'VAL-SURVIVAL inject: a declared gate tripping a floor (peel-syntax) the planner command does not is SKIPPED (no repair-loop deadlock)');
    ck(validateMicroQueue(peelQ, {}).ok, 'VAL-SURVIVAL inject: the skip leaves a queue that still validates');

    // (f) WIRED through deconstruct() (single-architect path): the fake architect re-invents
    // the gate; the RETURNED plan carries the declared gate verbatim (mechanical, not advice).
    {
      const weakJson = '```json\n' + JSON.stringify(weakQ()) + '\n```';
      const r = await deconstruct(vMission, { dispatchFn: async () => ({ content: weakJson }), maxRepairs: 0 });
      ck(r.ok === true && r.queue.steps[0].validation_command === GATE, 'VAL-SURVIVAL wired (deconstruct): model re-invented the gate -> declared gate mechanically re-imposed before validation');
    }

    // (g) WIRED through the panel integrator: same guarantee on the panel path.
    {
      const weakJson = '```json\n' + JSON.stringify(weakQ()) + '\n```';
      const dispatch = async (seat) => seat.role === 'integrator' ? { content: weakJson, provider: 'test' } : groundedArch;
      const r = await deconstructPanel(vMission, { dispatchFn: dispatch, maxRepairs: 0, route: 'panel' });
      ck(r.ok === true && r._panel === true && r.queue.steps[0].validation_command === GATE, 'VAL-SURVIVAL wired (panel integrator): declared gate re-imposed on the panel path too');
    }
  }
