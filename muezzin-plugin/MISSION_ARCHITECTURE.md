...
- **Small changes to already-authored files use `[command]` targeted-replace, not a whole-file `[edit]` re-emit.** When a step's real work is to change ONE line (or a few) in an existing `.txt`/`.md`/`.mission.txt`/config file, author it as a `[command]` step running a targeted `-replace` (PowerShell `(Get-Content f -Raw) -replace '<old>','<new>' | Set-Content f`) rather than an `[edit]` that re-emits the whole file. A targeted replace changes only the matched span and **structurally cannot** append out-of-scope content; a whole-file `[edit]` re-emit can silently balloon (engine-render-witness-maphtml-redirect appended 9 lines of debug garbage OUTSIDE scope to a `.mission.txt` exactly this way — undeclared growth that no containment/shrinkage guard catches, because it stays inside the allowlisted file). Reserve `[edit]` for authoring a file's full content; use `[command]` replace for surgical edits to authored substrate.
