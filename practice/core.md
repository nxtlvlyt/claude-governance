...
**Path B — Same turn (state file):** Write niyyah to `~/.claude/state/pending-niyyah-<session_id>.json` (session ID from `$env:CLAUDE_CODE_SESSION_ID` / `$CLAUDE_CODE_SESSION_ID`) via PowerShell/Bash before the Edit in the same turn. TTL: 60 seconds. Format: `{"ts": <unix-ms>, "niyyah_text": "niyyah:\n  source: ...\n  failure_mode: ...\n  work: ..."}`. The hook checks the session-scoped state file first, falling back to the unscoped `pending-niyyah.json` only when `session_id` is unavailable to it. **Session-scoped as of 2026-08-20**: the unscoped path let two concurrent Claude Code sessions on the same machine race on one shared file — reproduced live (an unrelated session's niyyah content silently overwrote another session's before its own hook check could read it back). Always include the session ID in the filename now. Surrender articulation follows the identical pattern: `~/.claude/state/pending-surrender-<session_id>.json` with `{"ts": <unix-ms>, "surrender_text": "surrender articulation:\n  substrate says: ..."}`.
