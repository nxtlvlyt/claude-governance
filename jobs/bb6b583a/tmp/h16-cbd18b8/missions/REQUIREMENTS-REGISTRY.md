...
|---|---|---|---|---|
| A1 | POI/roadside info richness (oddities, roadsides had no useful info) | popup fixes (fcc53e2, f6f9a71, 78e44f0) + **mt-enrich-scenic-osm.S1 (queued 2026-07-02)** | **PARTIAL (preview-verified, promotion owed)** | RENDER side DONE + preview-verified 19/19 e2e (cd3ad109 preview): oddity/charging/locker render real fields; scenic branch stopped fabricating (fake ★★★ default, 'Cool place' filler, wrong attribution — operator screenshot receipt) and shows an honest empty state. DATA side: scenic nodes carry only osm_id — mt-enrich-scenic-osm.S1 (lint-ok, queued) builds the Overpass tag overlay for the SK pilot bbox; S2 (national) authored on S1's landing. PRODUCTION still serves the old code — one operator word promotes the pre-verified preview. |
