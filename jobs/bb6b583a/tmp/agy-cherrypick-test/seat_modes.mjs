...
//
// ⛔ NO-CLOUD SUPERSESSION — FORK UPDATE 2026-07-07 (agy-muezzin step 2b): in the PARENT
// muezzin-plugin the operator's NO-CLOUD ruling (2026-07-02) removed the ollama-cloud lane
// structurally (2026-07-03) and every ollama-named seat dispatches nxtbeast-LOCAL only. THIS
// FORK is its own jurisdiction where Ollama Cloud IS permitted: seat_dispatch.mjs here now
// carries a restored ollama-cloud lane (native /api/chat + Bearer, fail-closed keyless), so
// cloud-censused names below (glm-5.2, deepseek-v4-pro, minimax-m3, kimi-k2.7-code,
// gemini-3-flash-preview, nemotron variants...) route to ollama.com via isCloudModelTag(),
// falling through to local/Claude on any lane failure. The parent plugin is unchanged.
//
...
// FORK-LOCAL route file (jurisdiction fix 2026-07-07: the inherited path pointed at the
// Claude-side state dir — the exact cross-boundary read the .agents rulebook forbids).
// Env override first so a beat harness can pin a mode without touching disk.
const ROUTE_FILE = process.env.AGY_MUEZZIN_ROUTE || 'C:/Users/marka/agy-muezzin/muezzin-route.json';

export const MODES = ['balance', 'anthropic-heavy', 'local-heavy', 'reasoning-heavy', 'gemini-heavy', 'claude-local-hybrid', 'agy-hybrid'];
...
const TABLE = {
  ...
  'agy-hybrid': {
    // AGY-SIBLING BUILD step 2 (2026-07-07, spec: the agy-implementation-research plan,
    // authored in the parent jurisdiction):
    // reasoning seats ride the Google Antigravity CLI on DISPLAY LABELS — the label IS the
    // --model id (agy_dispatch.mjs resolves labels verbatim; seat_dispatch's routePrefersAgy
    // routes any 8-label seat agy-first). The plan's pairing "conductor/integrator ->
    // Flash (High)/(Medium)" is only half seatable: the conductor is a session, not a TABLE
    // seat, so only the integrator lands here (Medium); Flash (High) remains the conductor's
    // own model outside this table.
    // ✅ CLOUD-NAMED SEATS (kimi-k2.7-code / glm-5.2 / deepseek-v4-pro / minimax-m3) are named
    // per the approved sibling plan and — since step 2b (2026-07-07) — actually REACH ollama.com:
    // this fork's seat_dispatch.mjs restored the ollama-cloud lane (native /api/chat + Bearer
    // from OLLAMA_API_KEY, fail-closed when keyless, one-shot TIMEOUT/NETWORK heals), gated by
    // isCloudModelTag() on the 2026-07-07 censused catalog. On any cloud-lane failure these
    // names fall through to local/Claude exactly as before. See the FORK UPDATE banner above.
    // EXECUTOR SEAT SETTLED 2026-07-08 (SOTA-search verdict, receipts in the research
    // agent's report + plan file): glm-5.2 1st (Terminal-Bench 2.1 = 81.0 independent,
    // SWE-bench Pro 62.1, 8 agent-harness validations, 128K max output vs kimi's 16K cap
    // — the cap is a real liability for full-file emission on big pages). kimi-k2.7-code
    // 2nd (vendor-only benchmarks, but won 14/15 on the one independent head-to-head) —
    // moved to the architect breadth slot so it keeps a live lane + 3-lab spread
    // (Google/Moonshot/DeepSeek). qwen3-coder 3rd (verified numbers but a generation
    // stale). Operator prior "kimi or glm > qwen-coder": SUPPORTED.
    architects: ['Gemini 3.1 Pro (High)', 'kimi-k2.7-code', 'deepseek-v4-pro'],
    integrator: 'Gemini 3.5 Flash (Medium)',                               // agy Flash — plan pairing (conductor gets High outside this table)
    executor: 'glm-5.2',                                                   // SOTA-verdict seat (see block above); kimi is the receipted fallback preference on executor-lane failure
    validator: 'GPT-OSS 120B (Medium)',                                    // agy open-weight verdict seat (producer != verifier, cross-lab diversity)
    auditor: 'minimax-m3',                                                 // plan: cloud minimax-m3 (see ⛔)
    final_auditor: 'north-mini-code-1.0:q4_K_M',                           // NOT in the plan mapping — kept LOCAL on the proven consensus seat (16/16 bench, same seat as claude-local-hybrid/local-heavy)
    witness: 'laguna-xs-2.1:q8_0',                                         // plan: nxtbeast laguna — KEEP LOCAL
  },
};
...
if (process.argv[1]?.endsWith('seat_modes.mjs') && process.argv.includes('--selftest')) {
  ...
  // 8. AGY-HYBRID: agy seats carry verified DISPLAY LABELS (the label is the --model id);
  //    witness stays local laguna; the mode is readable via readMode/pickSeat like any other.
  {
    const isAgyLabel = (m) => /^(Gemini 3\.[15] (Flash|Pro) \((Low|Medium|High)\)|Claude (Sonnet|Opus) 4\.6 \(Thinking\)|GPT-OSS 120B \(Medium\))$/.test(String(m));
    const s = resolveMode('agy-hybrid');
    ckT('agy-hybrid: architect A = Gemini 3.1 Pro (High) — a verified agy display label', s.architects[0] === 'Gemini 3.1 Pro (High)' && isAgyLabel(s.architects[0]));
    ck('agy-hybrid: architects B/C = cloud breadth seats (kimi swapped in when glm-5.2 took executor, 2026-07-08)', s.architects.slice(1), ['kimi-k2.7-code', 'deepseek-v4-pro']);
    ckT('agy-hybrid: integrator = Gemini 3.5 Flash (Medium) — display label', s.integrator === 'Gemini 3.5 Flash (Medium)' && isAgyLabel(s.integrator));
    ck('agy-hybrid: executor = glm-5.2 (SOTA-search verdict 2026-07-08)', s.executor, 'glm-5.2');
    ck('agy-hybrid: kimi keeps a live architect lane (breadth + fallback familiarity)', s.architects.includes('kimi-k2.7-code'), true);
    ckT('agy-hybrid: validator = GPT-OSS 120B (Medium) — display label', s.validator === 'GPT-OSS 120B (Medium)' && isAgyLabel(s.validator));
    ck('agy-hybrid: auditor = minimax-m3 (plan seat)', s.auditor, 'minimax-m3');
    ck('agy-hybrid: final_auditor = local consensus seat (not in plan mapping, kept local)', s.final_auditor, 'north-mini-code-1.0:q4_K_M');
    ck('agy-hybrid: witness = laguna (KEEP LOCAL per plan)', s.witness, 'laguna-xs-2.1:q8_0');
    ckT('agy-hybrid: NO seat is a bare Claude name (agy labels are not Claude-CLI names)', [...s.architects, s.integrator, s.executor, s.validator, s.auditor, s.final_auditor, s.witness].every((m) => !isClaude(m)));
    ck('readMode: MUEZZIN_MODE=agy-hybrid wins', readMode({ MUEZZIN_MODE: 'agy-hybrid' }, noRead), 'agy-hybrid');
    ck('readMode: route file mode=agy-hybrid -> agy-hybrid', readMode({}, () => JSON.stringify({ mode: 'agy-hybrid' })), 'agy-hybrid');
    ck('pickSeat: agy-hybrid validator -> GPT-OSS 120B (Medium)', pickSeat('validator', 'FB', { MUEZZIN_MODE: 'agy-hybrid' }, noRead), 'GPT-OSS 120B (Medium)');
  }

  // 9. LOCAL-ONLY SEATS: claude-local-hybrid's validator/auditor/witness are ALWAYS local-only;
