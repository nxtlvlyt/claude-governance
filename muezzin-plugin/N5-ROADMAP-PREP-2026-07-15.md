# N5 ROADMAP — WORKFLOW PREP + CRITIQUE (2026-07-15, conductor-committed)

STATUS: DESIGN PREP ONLY. The N5 BUILD remains gated per the operator's ratified
sequence (agy G3 -> graduation -> N5 with items 1-14 as spec). Produced by the
operator-suggested Sonnet workflow (wf_f9bce2be-fe0: 4 parallel readers -> synthesis
-> adversarial completeness critic; 6 agents, all sources read from disk).

OPERATOR-GATED CONFLICT, SURFACED PER THE FIFTH LAW (do not schedule through it):
the draft's Track A proposes starting items 1-7/10/12/14 NOW under the "next engine
batch" owner line — but ENGINE BATCH 2 (defined 2026-07-12, after that owner line)
enumerates W1-W6 + item-22 + item-23 and deliberately EXCLUDES items 1-14. Two
substrate layers conflict; the critic rates the draft's permissive reading a probable
gate violation (0.7). YOUR CALL governs when any 1-14 item starts. Nothing has been
built from this document.

PRE-BUILD FIX LIST (the critic's findings, owed against this draft before building):
(a) item 8 needs a full hook-surface inventory task; items 12/14 need fork-side
scheduling lines; QUEUE ITEM 18's rulings-bound content must join the plan.
(b) Track B's G3 premise must be re-probed against the live ladder at build time.
(c) five sourcing corrections incl. Law 6's code-mangle lint escalation owned nowhere.
(d) item 14's sketch must parameterize the FULL deploy guard chain + receipt shape.

=== ROADMAP DRAFT (verbatim workflow output) ===
...
=== COMPLETENESS CRITIQUE (verbatim, adversarial pass) ===
