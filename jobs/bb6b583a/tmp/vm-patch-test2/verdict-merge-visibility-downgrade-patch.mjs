...
const testOld = `  check(mergeVerdicts([{ seat: 'v', verdict: 'REVISE', findings: [{ id: 'A1', class: 'arkan' }, W('gap')] }]).consensus, 'REJECT', 'any arkan finding -> >= REJECT (a missed pillar invalidates; no expiation)');`;
const testNew = `  check(mergeVerdicts([{ seat: 'v', verdict: 'REVISE', findings: [{ id: 'A1', class: 'arkan' }, W('gap')] }]).consensus, 'REJECT', 'any arkan finding -> >= REJECT (a missed pillar invalidates; no expiation)');
