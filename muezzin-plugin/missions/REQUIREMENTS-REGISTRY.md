...
|---|---|---|---|---|
| A1 | POI/roadside info richness (oddities, roadsides had no useful info) | poi-tags, poi-services-nearby, poi-hover-preview-cards | **PARTIAL** | FIXED 2026-07-02 (conductor-direct, landed in HEAD, render-verified): `roadside_oddity`/`charging`/`locker` now have `popup()` branches rendering description/why-cool/detour, EV connector/fee, locker brand/hours — the empty-placeholder complaint is resolved for POIs that carry the data. STILL OPEN: the poi-tags/poi-services ENRICHMENT (adding tag/service data to POIs) is stranded on feature branches, not in HEAD (retros falsely DONE; poi-hover failed plan). |
