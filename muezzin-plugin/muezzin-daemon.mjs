...
// RESOLVED-STAMP VERIFICATION (Step 2, GAP-HUNT-2026-07-03 "RESOLVED-LANDED stamp is a pure-
// trust input"): before honoring a RESOLVED comment, check it against missionLandedState's
// byte-identity verdict. verdict !== 'FULL' -> STAMP-DISPUTED, the stamp is treated as absent
// (mission/dependency stays live). missionLandedState returning null (not a code-repo mission,
// unreadable file, or no ALLOW-FILES) -> fail-open, honor the stamp exactly as before this fix.
// Injectable (verifyStampFn) so queuedDepsHold itself stays PURE and offline-testable; the
// daemon wires the real fs+git implementation as the default.
function defaultVerifyStamp(depOrMissionPath) {
  try {
    const full = path.isAbsolute(depOrMissionPath) ? depOrMissionPath : path.join(HERE, String(depOrMissionPath).replace(/\//g, path.sep));
    const mtext = readFileSync(full, 'utf8');
    const st = missionLandedState(mtext, (repo, argstr) => {
      try { return { ok: true, out: execSync(`git -C "${repo}" ${argstr}`, { stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: 16 * 1024 * 1024 }).toString() }; } catch { return { ok: false, out: '' }; }
    });
    if (st === null) return null; // fail-open: not code-repo class / no ALLOW-FILES -> honor stamp
    return { disputed: st.verdict !== 'FULL', verdict: st.verdict };
  } catch { return null; } // unreadable mission file -> fail-open, honor stamp
}

export function queuedDepsHold(missionText, missionPath, autorunText, resultOkFn, verifyStampFn = defaultVerifyStamp) {
  const txt = String(missionText || '');
  const disputedStamps = [];
  ...
  // mt-c1-boundary: \b guards RESOLVED from matching inside UNRESOLVED (twin fix of
  // conduct-cycle.mjs closed()'s 2026-07-02 \b fix — this daemon regex was left unguarded).
  if (new RegExp(`^#.*\\bRESOLVED\\b.*${selfEsc}`, 'm').test(autorunText)) {
    const v = verifyStampFn(missionPath);
    if (v === null || !v.disputed) {
    ...
    disputedStamps.push({ dep: missionPath, verdict: v.verdict });
  }
  const deps = new Set();
  ...
  for (const dep of deps) {
    ...
    if (resolvedRe.test(autorunText)) {
      ...
      if (v === null || !v.disputed) resolved = true;
      else disputedStamps.push({ dep, verdict: v.verdict });
    }
    if (resolved) continue;                                                      // conductor-landed, stamp verified/fail-open
    if (doneRe.test(autorunText) && resultOkFn(dep) === true) continue;          // DONE + PASS receipt
    return { hold: true, dep, why: doneRe.test(autorunText) ? `dependency ${dep} is DONE but its result.json is not ok:true (hollow receipt)` : `dependency ${dep} not DONE/RESOLVED`, disputedStamps };
  }
  return { hold: false, disputedStamps };
}
    ...
    try {
      ...
      for (const { raw } of pending) {
        ...
        try {
          ...
          if (gate.disputedStamps && gate.disputedStamps.length) {
            for (const d of gate.disputedStamps) evt(`STAMP-DISPUTED: ${d.dep} RESOLVED stamp does not match missionLandedState verdict=${d.verdict} — treated as absent, mission stays live`);
          }
          if (gate.hold) {
...
if (process.argv.includes('--selftest')) {
  ...
  // ---- FIRE-TIME TARTIB GATE (queuedDepsHold — the 3-incident hollow-S2 class) ----
  {
    ...
    // mt-c1-boundary regression (daemon twin of conduct-cycle.mjs closed()'s 2026-07-02 \b
    // fix): an honest "still UNRESOLVED" annotation must NOT satisfy either the self-resolved
    // check or the dependency-resolved check.
    const arUnresolved = 'DONE missions/a.S1.mission.txt  <!-- t -->\n# still UNRESOLVED — investigating, do not treat as landed: missions/c.S1.mission.txt\n';
    ck(queuedDepsHold('MISSION-ID: x', 'missions/c.S2.mission.txt', arUnresolved, resOk).hold === true, 'mt-c1-boundary: dependency annotated "still UNRESOLVED" is NOT satisfied (no substring match into RESOLVED)');
    const unresSelf = queuedDepsHold('MISSION-ID: x', 'missions/c.S1.mission.txt', arUnresolved, resOk);
    ck(!(unresSelf.resolvedSelf === true), 'mt-c1-boundary: a mission itself annotated "still UNRESOLVED" is NOT self-retired');

    // RESOLVED-STAMP VERIFICATION fixtures (Step 2): a RESOLVED comment is only honored when
    // missionLandedState confirms it (verdict FULL) or cannot be determined (null, fail-open).
    const validStampFn = () => ({ disputed: false, verdict: 'FULL' });
    const disputedStampFn = () => ({ disputed: true, verdict: 'PARTIAL' });
    const nullStampFn = () => null;
    ck(queuedDepsHold('MISSION-ID: x', 'missions/c.S2.mission.txt', ar, resOk, validStampFn).hold === false, 'stamp-fixture: valid stamp (verdict FULL) is honored — dependency satisfied');
    const disputedGate = queuedDepsHold('MISSION-ID: x', 'missions/c.S2.mission.txt', ar, resOk, disputedStampFn);
    ck(disputedGate.hold === true && disputedGate.disputedStamps?.some((d) => d.dep === 'missions/c.S1.mission.txt'), 'stamp-fixture: disputed stamp (verdict != FULL) keeps the mission LIVE — dependency not satisfied, STAMP-DISPUTED recorded');
    ck(queuedDepsHold('MISSION-ID: x', 'missions/c.S2.mission.txt', ar, resOk, nullStampFn).hold === false, 'stamp-fixture: missionLandedState returning null fails OPEN — the stamp is honored');
  }
