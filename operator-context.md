...
5. Dispatch **qwen3.6:27b** (deep-dive — investigates gemma concerns + SOTA, second) via Python streaming. `"think": False` must be a **top-level body key** (NOT inside `options`) — without this, qwen emits chain-of-thought before JSON causing parse failures. Include SearxNG + gemma concerns.
6. Check api/ps.
